...
class User_model extends CI_Model {
    ...
    public function isTokenExpired($user_id)
    {
        ...
        $q = $this->db->order_by('created', 'DESC')->get_where('tokens', array('tokens.user_id' => $user_id), 1);
        $row = $q->row();
        $created = $row->created;
        $date_token = date('Y-m-d H:i:s', strtotime($created. ' + 1 hours'));
        $now = date('Y-m-d H:i:s');

        //return $q;
        //return $now . "       " . $date_token;

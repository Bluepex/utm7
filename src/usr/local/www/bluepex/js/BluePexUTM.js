$(function() {
	// keeps current sub-menu opened and closes the others
	var sidenav = $(".side-nav > li > a[data-toggle='collapse']");
	sidenav.click(function() {
		var current_menu = $(this).attr("data-target");
		sidenav.each(function() {
			if ($(this).parent().find("ul").hasClass("in") &&
			    $(this).attr("data-target") != current_menu) {
...
$('.icons-sociais img.icon').tooltip();

$(document).ready(function () {
   	$("#wizard").aiiaWizard();
	$("#wizard .nav li:first a").click(function() {
		$("#wizard").aiiaWizard('previous');
	});	
	$("#wizard .list-group-item").click(function() {

		var item = $(this).attr('data-category')

		$('#current_file').val(item);
	...
	function getProgressButtonsTemplate($steps, settings) {
		...
		var $progessButtonsWrapper = $("" +
			"<div class='aiia-wizard-progress-buttons-wrapper row'>" +
				"<div class='col-md-12'>" +
					"<ul class='nav nav-pills nav-justified aiia-wizard-progress-buttons-placeholder' style='border:0;margin-bottom:0'>" +
					"</ul>" +
				"</div>" +
			"</div>" +
		"").css({
		...
		$.each($steps, function (i, step) {
			...
			var $progressButton = $("" +
				"<li data-position='" + $step.attr('data-position') + "' style='width:49% !important;'><a href='#'><h1>" + $step.data('position') + ". </h1>" + $step.find('h1').html() + "</a></li>" +
			"");

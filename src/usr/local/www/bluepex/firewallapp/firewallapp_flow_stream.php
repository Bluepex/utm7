...
if ($_POST['save_os_policy']) {
	if ($_POST['eng_id'] != "") {
		...
		if (!$input_errors) {
			...
			write_config("firewallapp pkg: saved new Host_OS_Policy engine.");
			header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
			header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
			header( 'Cache-Control: no-store, no-cache, must-revalidate' );
			header( 'Cache-Control: post-check=0, pre-check=0', false );
			header( 'Pragma: no-cache' );
			header("Location: firewallapp_flow_stream.php?id=$id");
			exit;
		}
	}
}
...
elseif ($_POST['del_os_policy']) {
	...
	if (isset($id) && $a_nat[$id]) {
		$a_nat[$id] = $natent;
		write_config("firewallapp pkg: deleted a Host_OS_Policy engine.");
	}
	header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
	header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
	header( 'Cache-Control: no-store, no-cache, must-revalidate' );
...
elseif ($_POST['save'] || $_POST['apply']) {
	...
	if (!$input_errors) {
		...
		/* then update the firewallapp.conf file for this	*/
		/* interface.									 */
		/**************************************************/
		if (isset($id) && $a_nat[$id]) {
			$a_nat[$id] = $natent;
			$if_friendly = convert_friendly_interface_to_friendly_descr($a_nat[$id]['interface']);
			$if = get_real_interface($a_nat[$id]['interface']);
			$uuid = $a_nat[$id]['uuid'];
			bp_write_report_db("report_0008_acp_fapp_stream", "FirewallApp|{$if_friendly}|{$if}{$uuid}");

			write_config("firewallapp pkg: saved flow or stream configuration changes.");
			$rebuild_rules = false;
...
elseif ($_POST['save_import_alias']) {
	...
	if ($_POST['mode'] =='add_edit_os_policy') {
		...
		if (!$input_errors) {
			...
			write_config("firewallapp pkg: saved Host_OS_Policy engine created from a defined firewall alias.");
			$importalias = false;
			$selectalias = false;
			header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
			header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
...
$pglinks = array("", "/firewallapp/firewallapp_interfaces.php", "/firewallapp/firewallapp_interfaces_edit.php?id={$id}", "@self");
$pgtitle = array("Services", "FirewallApp", "Interface Settings", "{$if_friendly} - Flow and Stream Engine");
...
<?php
	if ($importalias) {
		...
		print('<form action="firewallapp_flow_stream.php" method="post" name="iform" id="iform" class="form-horizontal">');
		...
		include("/usr/local/www/firewallapp/firewallapp_import_aliases.php");
		print('</form>');

	} elseif ($add_edit_os_policy) {

		$form = new Form(false);
		include("/usr/local/www/firewallapp/sfirewallapp_os_policy_engine.php");

	} else {
?>

<form action="firewallapp_flow_stream.php" method="post" name="iform" id="iform" class="form-horizontal">
...
$section = new Form_Section(gettext('IP Defragmentation'),'',0,true);
...
),true)->setHelp('Number of defragmented flows to follow. Default is 65,535 fragments. Sets the number of defragmented flows to follow for reassembly.');
$section->addInput(new Form_Input(
	'ip_max_frags',
	'Max Fragments',
	'text',
	$pconfig['ip_max_frags']
),true)->setHelp('Maximum number of IP fragments to hold. Default is 65,535 fragments. Sets the maximum number of IP fragments to retain in memory while awaiting reassembly. This must be equal to or greater than the Max Trackers value specified above.');
$section->addInput(new Form_Input(
	'frag_hash_size',
	'Fragmentation Hash Table Size',
	'text',
	$pconfig['frag_hash_size']
),true)->setHelp('Hash Table size. Default is 65,536 entries. Sets the size of the Hash Table used by the defragmentation engine.');
$section->addInput(new Form_Input(
	'ip_frag_timeout',
	'Timeout',
	'text',
	$pconfig['ip_frag_timeout']
),true)->setHelp('Max seconds to hold an IP fragement. Default is 60 seconds. Sets the number of seconds to hold an IP fragment in memory while awaiting the remainder of the packet to arrive.');
$form->add($section);
...
),true)->setHelp('Hash Table size used by the flow engine. Default is 65,536 entries.');
$section->addInput(new Form_Input(
	'flow_prealloc',
	'Preallocated Flows',
	'text',
	$pconfig['flow_prealloc']
),true)->setHelp('Number of preallocated flows ready for use. Default is 10,000 flows.');
$section->addInput(new Form_Input(
	'flow_emerg_recovery',
	'Emergency Recovery',
	'text',
	$pconfig['flow_emerg_recovery']
),true)->setHelp('Percentage of preallocated flows to complete before exiting Emergency Mode. Default is 30%.');
$section->addInput(new Form_Input(
	'flow_prune',
	'Prune Flows',
	'text',
	$pconfig['flow_prune']
),true)->setHelp('Number of flows to prune in Emergency Mode when allocating a new flow. Default is 5 flows.');
$form->add($section);

$section = new Form_Section(gettext('Flow Timeout Settings'),'',0,true);
...
))->setHelp('Max memory to be used by stream engine. Default is 131,217,728 bytes (128MB). Sets the maximum amount of memory, in bytes, to be used by the stream engine. This number will likely need to be increased beyond the default value in systems with more than 4 processor cores. If firewallapp fails to start and logs a memory allocation error, increase this value in 4 MB chunks until firewallapp starts successfully.');
$section->addInput(new Form_Input(
	'stream_prealloc_sessions',
	'Preallocated Sessions',
	'text',
	$pconfig['stream_prealloc_sessions']
),true)->setHelp('Number of preallocated stream engine sessions. Default is 32,768 sessions. Sets the number of stream engine sessions to preallocate. This can be a performance enhancement.');
$section->addInput(new Form_Checkbox(
	'enable_midstream_sessions',
	'Enable Mid-Stream Sessions',
	'firewallapp will pick up and track sessions mid-stream. Default is Not Checked.',
...
$section->addInput(new Form_Checkbox(
	...
	'firewallapp will track asynchronous one-sided streams. Default is Not Checked.',
...
),true)->setHelp('Max memory to be used for stream reassembly. Default is 131,217,728 bytes (128MB). Sets the maximum amount of memory, in bytes, to be used for stream reassembly.');
$section->addInput(new Form_Input(
	'reassembly_depth',
	'Reassembly Depth',
	'text',
	$pconfig['reassembly_depth']
),true)->setHelp('Amount of a stream to reassemble. Default is 1,048,576 bytes (1MB). Sets the depth, in bytes, of a stream to be reassembled by the stream engine. Set to 0 (unlimited) to reassemble entire stream. This is required for file extraction.');
$section->addInput(new Form_Input(
	'reassembly_to_server_chunk',
	'To-Server Chunk Size',
	'text',
	$pconfig['reassembly_to_server_chunk']
),true)->setHelp('Size of raw stream chunks to inspect. Default is 2,560 bytes. Sets the chunk size, in bytes, for raw stream inspection performed for \'to-server\' traffic.');
$section->addInput(new Form_Input(
	'reassembly_to_client_chunk',
	'To-Client Chunk Size',
	'text',
	$pconfig['reassembly_to_client_chunk']
),true)->setHelp('Amount of a stream to reassemble. Default is 2,560 bytes. Sets the chunk size, in bytes, for raw stream inspection performed for \'to-client\' traffic.');
$section->addInput(new Form_Input(
	'max_synack_queued',
	'Max different SYN/ACKs to queue',
	'number',
	$pconfig['max_synack_queued']
),true)->setHelp('Sets max number of extra SYN/ACKs firewallapp will queue and delay judgement on while awaiting proper ACK for 3-way handshake. Default is 5.');
$form->add($section);
...
<div class="infoblock">
	<?=print_info_box('<strong>Note:</strong> Please save your settings before you exit. Changes will rebuild the rules file. This may take several seconds. firewallapp must also be restarted to activate any changes made on this screen.', 'info')?>
</div>

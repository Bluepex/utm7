...
function addClogBlock($sid, $if_real, $uuid, $gidtable) {
	if (file_exists("/usr/local/etc/suricata/suricata_{$uuid}_{$if_real}/rules/suricata.rules")) {
		$grepInfoSID=shell_exec("grep -r 'sid:{$sid};' /usr/local/etc/suricata/suricata_{$uuid}_{$if_real}/rules/suricata.rules | head -n1");
		if (!empty($grepInfoSID)) {
			$dateNow = date("m/d/Y-H:m:s.100000");
			$classType=trim(explode(";",explode("classtype:", $grepInfoSID)[1])[0]);
			$rev=trim(explode(";",explode("rev:", $grepInfoSID)[1])[0]);
            ...
            $gid=trim(explode(";",explode("gid:", $grepInfoSID)[1])[0]);
            if (empty($gid)) {
                $gid=1;
            }
...
function configTableIptable($gidtable, $sid, $if_real, $uuid, $action, $ips_mode) {
		...
		if (file_exists($gidtable)) {
			...
			foreach(explode("\n", trim(shell_exec("pfctl -sT | grep fapp2c_{$if_real}{$uuid}"))) as $tablesSimple) {
				if (!empty($tablesSimple)) {
					...
					if ($action == "add") {
						addClogBlock($sid, $if_real, $uuid, $gidtable);
					}
				}
			}
			// with exceptions
			foreach(explode("\n", trim(shell_exec("/sbin/pfctl -sT | grep fapp2c_b_{$if_real}{$uuid}_1{$sid}"))) as $tablesSimple) {
				if (!empty($tablesSimple)) {
...
if (isset($_POST['state']) && !empty($_POST['state'])) {
	...
	for($counter=0;$counter<=(count($_POST['gid'])-1);$counter++) {
		...
		$sid = $_POST['sid'][$counter];
		$action = $_POST['state'][$counter];
		$currentfile = $_POST['currentfile'][$counter];
		$ip_source = empty($_POST['ip_source'][$counter]) ? 'any' : $_POST['ip_source'][$counter];
		$port_source = empty($_POST['port_source'][$counter]) ? 'any' : $_POST['port_source'][$counter];
		$direction = $_POST['direction'][$counter];
		$ip_destination = empty($_POST['ip_destination'][$counter]) ? 'any' : $_POST['ip_destination'][$counter];
		$port_destination = empty($_POST['port_destination'][$counter]) ? 'any' : $_POST['port_destination'][$counter];		
		...
		switch ($action) {
			...
			case "pass":		
				...
				if (!is_array($passid[$gid][$sid])) {
					$passid[$gid][$sid] = array();
				}		
		...
		// Clear ALL block table
		if ($action == "pass") {
			foreach(explode("\n", trim(shell_exec("pfctl -sT | grep fapp2c | grep -Ev '_b_|_p_'"))) as $tablesSimple) {
		...
		if ($action == "alert") {
			foreach(explode("||", $a_rule[$id]['rule_sid_force_alert']) as $line_now) {
				$gid_sid = explode(":", $line_now)[1];
				if ((intval($gid_sid) == intval($sid)) && (!in_array($sid, $sid_used))) {
					$sid_used[] = $sid;
					if (!in_array($sid, $suricata_sig_gid_ignore)) {
						if (!in_array($sid, $suricata_sig_gid_ignore) && ($a_rule[$id]['ips_mode'] == 'ips_mode_legacy')) {
							configTableIptable('/usr/local/share/suricata/rules/sid:' . $sid .'.txt', $sid, $if_real, $if_uuid, "delete", $ips_mode);
						} elseif (in_array($sid, $suricata_sig_gid_ignore) && ($a_rule[$id]['ips_mode'] == 'ips_mode_mix')) {
							configTableIptable('/usr/local/share/suricata/rules/sid:' . $sid .'.txt', $sid, $if_real, $if_uuid, "delete", $ips_mode);
						}
					}
				}
		...
		if ($action == "drop") {
			foreach(explode("||", $a_rule[$id]['rule_sid_force_drop']) as $line_now) {
				...
				if ((intval($gid_sid) == intval($sid)) && (!in_array($sid, $sid_used))) {
					...
					if (!in_array($sid, $suricata_sig_gid_ignore)) {
						if (!in_array($sid, $suricata_sig_gid_ignore) && ($a_rule[$id]['ips_mode'] == 'ips_mode_legacy')) {
							configTableIptable('/usr/local/share/suricata/rules/sid:' . $sid .'.txt', $sid, $if_real, $if_uuid, "add", $ips_mode);
						} elseif (in_array($sid, $suricata_sig_gid_ignore) && ($a_rule[$id]['ips_mode'] == 'ips_mode_mix')) {
							configTableIptable('/usr/local/share/suricata/rules/sid:' . $sid .'.txt', $sid, $if_real, $if_uuid, "add", $ips_mode);
						}
					}

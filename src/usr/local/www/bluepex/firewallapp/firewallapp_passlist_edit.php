...
if ($_POST['cancel']) {
	header("Location: /firewallapp/firewallapp_passlist.php");
	exit;
}
...
if (is_null($id)) {
	#header("Location: /firewallapp/suricata_interfaces_passlist.php");
	header("Location: /firewallapp/firewallapp_passlist.php");
	exit;
}
...
if ($_POST['save']) {
	...
	if ($_POST['address']) {
		...
		if (is_alias($_POST['address']) && trim(filter_expand_alias($_POST['address'])) == "")
			$input_errors[] = gettext("FQDN aliases are not supported in FirewallAPP.");
	}
	if (!$input_errors) {
		$p_list = array();
		/* post user input */
		...
		write_config("FirewallAPP pkg: modified PASS LIST {$p_list['name']}.");

		/* create pass list and homenet file, then sync files */
		sync_suricata_package_config();
		
		$savemsg = gettext("Attention, the Entered IP will not be monitored by the Firewall APP in the rules with BLOCKED Status!");
...
$pgtitle = array(gettext("Services"), gettext("FirewallApp"), gettext("Pass List"), gettext("Pass List Edit"));
$pglinks = array("", "./firewallapp/services.php", "./firewallapp/firewallapp_passlist.php", "@self");
...
$tab_array[] = array(gettext("Pass Lists"), true, "/firewallapp/firewallapp_passlist.php");
...
$form = new Form(FALSE);
$section = new Form_Section(gettext('General Information'));
$section->addInput(new Form_Input(
	'name',
	gettext('Name'),
	'text',
	$pconfig['name']
))->setPattern('[a-zA-Z0-9_]+')->setHelp(gettext('The list name may only consist of the characters "a-z, A-Z, 0-9 and _".'));
$section->addInput(new Form_Input(
	'descr',
	gettext('Description'),
	'text',
	$pconfig['descr']
))->setHelp(gettext('You may enter a description here for your reference.'));
$form->add($section);

$section = new Form_Section(gettext('Auto-Generated IP Addresses'),'',0,true);
$section->addInput(new Form_Checkbox(
	'localnets',
	gettext('Local Networks'),
	gettext('Add firewall Locally-Attached Networks to the list (excluding WAN).  Default is checked (but see warning below).'),
	$pconfig['localnets'] == 'yes' ? true:false,
	'yes'
), true)->setHelp(gettext('If creating a custom HOME_NET list, then this box should usually be checked.'));
$section->addInput(new Form_Checkbox(
	'wanips',
	gettext('WAN IP'),
	gettext('Add WAN interface IP to the list.  Default is checked).'),
	$pconfig['wanips'] == 'yes' ? true:false,
	'yes'
), true)->setHelp(gettext('If creating a custom HOME_NET list and using NAT, then this box should usually be checked.'));
$section->addInput(new Form_Checkbox(
	'wangateips',
	gettext('WAN Gateways'),
	gettext('Add WAN Gateways to the list.  Default is checked.'),
	$pconfig['wangateips'] == 'yes' ? true:false,
	'yes'
), true);
$section->addInput(new Form_Checkbox(
	'wandnsips',
	gettext('WAN DNS Servers'),
	gettext('Add WAN DNS servers to the list.  Default is checked.'),
	$pconfig['wandnsips'] == 'yes' ? true:false,
	'yes'
), true);
$section->addInput(new Form_Checkbox(
	'vips',
	gettext('Virtual IP Addresses'),
	gettext('Add Virtual IP Addresses to the list.  Default is checked.'),
	$pconfig['vips'] == 'yes' ? true:false,
	'yes'
), true);
$section->addInput(new Form_Checkbox(
	'vpnips',
	gettext('VPN Addresses'),
	gettext('Add VPN Addresses to the list.  Default is checked.'),
	$pconfig['vpnips'] == 'yes' ? true:false,
	'yes'
), true)->setHelp(gettext('If creating a custom HOME_NET list, then this box should usually be checked.'));
$form->add($section);

$section = new Form_Section('add an IPs object to the PassList');
$group = new Form_Group(gettext('Assigned Alias'));
$group->add(new Form_Input(
	'address',
	gettext('Assigned Alias'),
	'text',
	$pconfig['address']

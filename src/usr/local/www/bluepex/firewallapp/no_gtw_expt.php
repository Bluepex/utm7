...
define("IF_EX_LAN", "{$g['etc_path']}/if_ex_lan.conf");

if ($_POST['save']) {
	$exceptions_in_file = file_exists(IF_EX_LAN) ? array_unique(array_filter(explode(",", file_get_contents(IF_EX_LAN)))) : [];
	$exceptions_post = [];
	...
	foreach ($out_interfaces as $key => $suricata_interface) {
		...
		write_config("Bluepex: FirewallApp: Remove mixed mode {$if}!!!");
	}
	...
	if (!empty($remove_exceptions)) {
		bp_write_report_db("report_0008_acp_fapp_no_gtw_fapp_to_acp", implode(",", $remove_exceptions));
	}

	if (!empty($add_exceptions)) {
		bp_write_report_db("report_0008_acp_fapp_no_gtw_acp_to_fapp", implode(",", $add_exceptions));
	}

	file_put_contents(IF_EX_LAN, $_POST['exceptions']);
...
$pgtitle = array(gettext("FirewallApp"), gettext("Exceptions"));
...
<div class="infoblock">
	<div class="alert alert-info clearfix" role="alert">
		<div>
			<p><?=gettext("Exceptions will no longer be part of the FirewallApp application")?></p>
...
$section = new Form_Section(gettext('Configuring Exceptions for Interfaces Without Gateways'));
...
))->setHelp(gettext('List of Exceptions for interfaces that do not have gateways. Ex: em0,em1'));
...
$section->addInput(new Form_StaticText(
	gettext('Interfaces'),
	generate_buttons()
))->setHelp(gettext('Select the interfaces to make exception in FirewallApp'));

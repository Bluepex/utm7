...
if (count($_REQUEST) == 1) {
	header("Location: pkg_mgr_install.php?id=firmware");
	return;
}
...
if ($firmwareupdate) {
	...
	$text_title = (file_exists("/etc/suite_update_enabled")) ? gettext("Pack Fix") : gettext("Cpack's");
	$tab_array[] = array($text_title, false, "bluepex_cpack.php");
} else {
...
<?php if ($firmwareupdate): ?>
	<div style='margin: auto; text-align:end;' id="gearActions">
		<img style='width: 32px;' src='./images/bp-gear.png' onclick="mostrarCampoVersion();" id="changeImg">
	</div>
	<div class="modal fade" id="mostrarCampoVersion" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="TituloModalCentralizado"><?=gettext("Release firmware update field")?> </h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form method="POST" action="" style="margin-top: 10px !important;border:unset;">
						<div class="form-row">
							<div class="col">					
...
<form action="pkg_mgr_install.php" method="post" class="form-horizontal" id="pkg_mgr_install">
<?php
...
?>
				...
				<button type="click" class="btn btn-success" name="pkgconfirm" id="pkgconfirm" value="<?=gettext("Confirm")?>">
					<i class="fa fa-check icon-embed-btn"></i>
					<?=gettext("Confirm")?>
				</button>

				<!-- modal confirm install new pack -->
				<div class="modal fade" id="modalPermitedInstall" tabindex="-1" role="dialog" aria-labelledby="modalPermitedInstall" aria-hidden="true">
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="TituloModalCentralizado"><?=gettext("Confirmar instalação")?> </h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<form method="POST" action="" style="margin-top: 10px !important;border:unset;">
									<div class="form-row">
										<div class="col">					
											<label for="recipient-name" class="col-form-label"><?=gettext("Enter password:")?></label>
											<input type="password" class="form-control" id="passInstall" name="passInstall" maxlength="50">
										</div>
									</div>
									<hr>
									<button type="button" class="btn btn-secondary" data-dismiss="modal"><?=gettext("Close")?></button>
									<button type="submit" name="confirm_pass_install" id="confirm_pass_install" class="btn btn-primary"><?=gettext("Continue")?></button>
								</form>
							</div>
						</div>
					</div>
...
?>
<script>
function mostrarCampoVersion() {
	if (!!document.getElementsByClassName("form-group col-11 left-border-blue box-white p-6")[0]) {
		if (document.getElementsByClassName("form-group col-11 left-border-blue box-white p-6")[0].style.display == "block") {
			...
			document.getElementById("changeImg").src="./images/bp-gear.png";
		} else {
			$('#mostrarCampoVersion').modal('show');
		}
	}
}


function permitInstallModal() {
	$('#modalPermitedInstall').modal('show');
}

$(document).ready(function () {
	$("#confirmar_senha_liberar_modal").click(function (event) {
		...
		success: function(data) {
			if (data == "ok") {
				document.getElementsByClassName("form-group col-11 left-border-blue box-white p-6")[0].style.display="block";
				document.getElementsByClassName("panel-heading")[0].style.display="block";
				document.getElementById("changeImg").src="./images/bp-logout.png";
				document.getElementById('senhaLiberar').value="";
				$('#mostrarCampoVersion').modal('hide');
			} else {
				$("#senhaLiberar").val("");
	...
	$("#pkgconfirm").click(function (event) {
		permitInstallModal();
		event.preventDefault();
	});

	$("#confirm_pass_install").click(function (event) {
		event.preventDefault();
		$.ajax({
		data: { pass_modal: $('#passInstall').val() },
		method: "post",
		url: "ajax_pass_modal.php",
		success: function(data) {
			if (data == "ok") {
				$("#pkg_mgr_install").submit();
			} else {
				$("#passInstall").val("");
				alert("<?=gettext("Incorrect password!")?>");
			}

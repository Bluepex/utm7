...
class Reports extends CI_Controller
{
	...
	public function index()
	{
		$data = [
			"reports" => getReports(),
			"periods" => getPeriods(),
			"users" => $this->relatoriosapi->getUsersFromDatabase(),
			"users_config" => $this->relatoriosapi->getUsersFromConfigXML(),
			"groups" => $this->relatoriosapi->getGroupsFromDatabase(),
		];
	...
	public function listReportsFiles()
	{
		...
		if (!empty($files)) {
			foreach ($files as $file) {
				...
				$desc_report = getReports($report);
				if (!is_array($desc_report)) {
					$report_files[] = [
						"datetime" => date("d/m/Y H:i:s", $datetime),
						"file" => $filename,
						"filesize" => filesize($file),
						"report" => $report . " - " . $desc_report,
					];
				}
			}
		}

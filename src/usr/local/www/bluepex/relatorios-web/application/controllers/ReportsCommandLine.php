...
class ReportsCommandLine extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if (!empty($this->utm)) {
			...
			$this->load->library("RelatoriosApi/RelatoriosApi", $conn);
		} else {
	...
	public function report($schedule_id)
	{
		...
		$result = $this->relatoriosapi->getReport($params);
		...
		if ($params['format'] == "csv") {
			...
			if (is_resource($fp)) {
				if ($params['report'] == "P0001") {
					...
					$first_line = ['ID da Regra', 'Ação', 'Regra', 'Classificação', 'IP Origem', 'Direção', 'IP Destino'];
					fputcsv($fp, $first_line);
					foreach ($result->data as $res) {
						$line = [ $res->id_rule, $res->action, $res->rule, $res->classification, $res->src_ip_port, $res->dir, $res->dst_ip_port ];
						fputcsv($fp, $line);
					}
				} elseif ($params['report'] == "P00061") {
					$total = 0;
					foreach ($result->data as $res) {
						$total = $total+$res->value;
					}
					$first_line = ['Data/Hora', 'Assinatura', 'IP', 'Usuário', 'Ação'];
					fputcsv($fp, $first_line);
					foreach ($result->data as $res) {
						$line = [ $res->time_date, $res->rule, $res->src_ip_port, $res->src_ip_port, $res->action ];
						fputcsv($fp, $line);
					}
					...
					$first_line = [ $this->lang->line('reports_cmd_username'), $this->lang->line('reports_cmd_rem_ip'), $this->lang->line('reports_cmd_local_ip'), $this->lang->line('reports_c
md_port'), $this->lang->line('reports_cmd_bytes_send'), $this->lang->line('reports_cmd_bytes_send'), $this->lang->line('reports_cmd_time_connect'), $this->lang->line('reports_cmd_time_disconnect') ];
					fputcsv($fp, $first_line);
					foreach ($result->data as $res) {

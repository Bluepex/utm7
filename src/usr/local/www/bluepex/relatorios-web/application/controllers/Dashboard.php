	...
	public function makeGrid_RT0004($realtime_data)
	{
		...
		foreach($realtime_data as $rules) {
			foreach($rules as $obj) {
				...
				if (isset($user_agents_captive_portal[trim($obj->user_agent)])) {
					$user_agent = "<img style='height:30px' src='" . base_url($user_agents_captive_portal[$obj->user_agent]) . "' alt='{$obj->user_agent}' title='
{$obj->user_agent}' />";
				} else {
				...
				$html .= "<tr>";
				$html .= "<td>" . htmlentities($user) . "</td>";
				$html .= "<td>{$social_login}</td>";
				$html .= "<td class='text-center'>{$user_agent}</td>";
				$html .= "<td>{$obj->ip}</td>";
				$html .= "<td>{$obj->mac}</td>";
				$html .= "<td>{$obj->connect_start}</td>";
				$html .= "<td>{$obj->last_activity}</td>";
				$html .= "</tr>";
			}
		}
		$html .= "</tbody>";

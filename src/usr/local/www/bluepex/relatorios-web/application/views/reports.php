											...
											<form id="FormPre" class="form-horizontal" action="<?=base_url() . "reports/generate"?>" method="POST">
												...
												<div class="col-md-8" id="preprocessed_inputs">
													...
													<div class="form-group">
														...
														<div class="col-md-8">
															<!--<input type="number" name="limit" class="form-control" min="1" max="500" />-->
										...
										<div class="panel-body">
											<form id="FormCustom" class="form-horizontal" action="<?=base_url() . "reports/generate"?>" method="POST">
												...
												<div class="col-md-8" id="export_inputs">
													...
													<div class="form-group">
														<label for="periodAdvanced" class="col-sm-4 control-label"><?=$this->lang->line('reports_input_period');?></label>
														<div class="col-sm-8">
															<select class="form-control" id="periodAdvanced" name="periodAdvanced">
																<?php
																$periods['twomonths'] = '60 dias';
																$periods['threemonths'] = '90 dias';
																?>
																<?php foreach ($periods as $id => $period) : ?>
																<option value="<?=$id?>"><?=$period?></option>
																<?php endforeach; ?>
													...
													<input type="text" id="interval_from" name="interval_from" class="form-control datetimepicker" value="" style="display:none;"/>
													<input type="text" id="interval_until" name="interval_until" class="form-control datetimepicker" value="" style="display:none;"/>
													...
													<div class="form-group" style="display:none">
														...
														<div class="col-md-8">
															<select id="username_db" name="username[]" class="form-control">
																<option value=""></option>
																<?php foreach ($users as $user) : ?>
																<option value="<?=$user?>"><?=$user?></option>
																<?php endforeach; ?>
															</select>
															<select id="username_config_xml" name="username[]" class="form-control">
                                                                <option value=""></option>
                                                                <?php foreach ($users_config as $user_config) : ?>
							...
							<form id="FormCustom" class="form-horizontal" action="<?=base_url() . "reports/generate"?>" method="POST">
								...
								<div class="col-md-8" id="export_inputs">
									...
									<div class="form-group" id="interval">
										...
										<label for="interval" class="col-sm-4 control-label"><?=$this->lang->line('reports_input_interval');?></label>
										<div class="col-sm-8">
											<select class="form-control" id="periodAdvanced1" name="periodAdvanced1">
												<?php foreach ($periods as $id => $period) : ?>
													<option value="<?=$id?>"><?=$period?></option>
												<?php endforeach; ?>
											</select>
										</div>
										<input type="text" id="interval_from1" name="interval_from" class="form-control datetimepicker" value="" style="display:none;"/>
										<input type="text" id="interval_until1" name="interval_until" class="form-control datetimepicker" value="" style="display:none;"/>
									</div>
									<div class="form-group" style="display:none">
										<label for="username" class="col-sm-4 control-label"><?=$this->lang->line('reports_input_user');?></label>
										<div class="col-md-8">
...
$('#periodAdvanced').change(function() {
	var value = $(this).val();
	setFieldDate(value);
});
$('#periodAdvanced1').change(function() {
	var value = $(this).val();
	setFieldDate1(value);
});
function setFieldDate(value) {
	<?php $dateNow = date("Y-m-d H:i:s"); ?>
	$("#periodAdvanced").val(value);
	if (value == "daily") {
		$("#interval_until").val("<?=$dateNow?>");
		$("#interval_from").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -1 day"))?>");
	} else if (value == "weekly") {
		$("#interval_until").val("<?=$dateNow?>");
		$("#interval_from").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -7 day"))?>");
	} else if (value == "monthly") {
		$("#interval_until").val("<?=$dateNow?>");
		$("#interval_from").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -30 day"))?>");
	} else if (value == "twomonths") {
		$("#interval_until").val("<?=$dateNow?>");
		$("#interval_from").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -60 day"))?>");
	} else if (value == "threemonths") {
		$("#interval_until").val("<?=$dateNow?>");
		$("#interval_from").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -90 day"))?>");
	}
}
function setFieldDate1(value) {
	<?php $dateNow = date("Y-m-d H:i:s"); ?>
	$("#periodAdvanced1").val(value);
	if (value == "daily") {
		$("#interval_until1").val("<?=$dateNow?>");
		$("#interval_from1").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -1 day"))?>");
	} else if (value == "weekly") {
		$("#interval_until1").val("<?=$dateNow?>");
		$("#interval_from1").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -7 day"))?>");
	} else if (value == "monthly") {
		$("#interval_until1").val("<?=$dateNow?>");
		$("#interval_from1").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -30 day"))?>");
	} else if (value == "twomonths") {
		$("#interval_until1").val("<?=$dateNow?>");
		$("#interval_from1").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -60 day"))?>");
	} else if (value == "threemonths") {
		$("#interval_until1").val("<?=$dateNow?>");
		$("#interval_from1").val("<?=date('Y-m-d H:i:s', strtotime("{$dateNow} -90 day"))?>");
	}
}
setTimeout(() => {
	setFieldDate('daily');
...
$(function () {
	...
	$(".btn-reports").click(function() {
		...
		// Switch username_db and username_config_xml
		if (report_id == "0003" || report_id == "0007") {
			$('#username_db').hide();
			$('#username_config_xml').show();
		} else {
			$('#username_config_xml').hide();
			$('#username_db').show();
		}
	...
	$("#FormPre, #FormCustom").submit(function(e) {
		...
		$.ajax({
			...
			success: function( response ) {
				...
				//location.reload();
				$('input[name="interval_from"]').val("");
				$('input[name="interval_until"]').val("");
				setFieldDate($("#periodAdvanced").val());
				setFieldDate1($("#periodAdvanced1").val());

				$('#username').val("");
				$('#username_db').val("");
				$('#username_config_xml').val("");
				$('input[name="ipaddress"]').val("");
			},

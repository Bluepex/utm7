					...
					<div class="panel-body">
						<div class="col-md-3">
							<div class="btn-group">
								...
								<ul class="dropdown-menu">
									...
									<li><a href="<?=base_url() . "utm/change-display/{$_utm->id}"?>"><?=$_utm->name?></a></li>
									<?php endforeach; ?>
								</ul>
							</div>
						</div>
						<div class="col-md-6">
							<div class="text-center" id="navbar">
								<a href="<?=base_url("dashboard/daily")?>" id="period_daily" class="btn btn-blue">
									<i class="fa fa-calendar"></i> <?=$this->lang->line('utm_dash_period_1');?>
								</a>
								<a href="<?=base_url("dashboard/weekly")?>" id="period_weekly" class="btn btn-blue">
									<i class="fa fa-calendar"></i> <?=$this->lang->line('utm_dash_period_7');?>
								</a>
								<a href="<?=base_url("dashboard/monthly")?>" id="period_monthly" class="btn btn-blue">
									<i class="fa fa-calendar"></i> <?=$this->lang->line('utm_dash_period_30');?>
								</a>
								<?php if (checkPermission("dataclick-web/dashboard/refresh-data")) : ?>
								<a href="<?=base_url("dashboard/refresh-data")?>" class="btn btn-warning btn-refresh" data-toggle="tooltip" data-placement="bottom" title="Refresh">
...
$(document).ready(function() {
	...
	var graph6_user = [];
	var graph6_bytes_sent = [];
	var graph6_bytes_rec = [];
	<?php foreach ($json->report_top_5_openvpn_control as $obj) : ?>
		graph6_user.push("<?php echo $obj->username ?>");
		graph6_bytes_sent.push("<?php echo $obj->bytes_sent ?>");
		graph6_bytes_rec.push("<?php echo $obj->bytes_received ?>");
	<?php endforeach; ?>
	var params = {
		"title_series1": '<?=$this->lang->line('utm_dash_graph_vpn_series1');?>',
		"title_series2": '<?=$this->lang->line('utm_dash_graph_vpn_series2');?>',
		"data": {
			"xAxis": graph6_user,
			"series1": graph6_bytes_sent,
			"series2": graph6_bytes_rec
		}
	}
	chartRT0006(document.getElementById('chart6'), params);
	<?php else : ?>

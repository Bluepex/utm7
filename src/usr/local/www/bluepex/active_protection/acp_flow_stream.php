...
if ($_POST['save_os_policy']) {
	if ($_POST['eng_id'] != "") {
		...
		if (!$input_errors) {
			...
			write_config("acp pkg: saved new Host_OS_Policy engine.");
			header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
			header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
			header( 'Cache-Control: no-store, no-cache, must-revalidate' );
			header( 'Cache-Control: post-check=0, pre-check=0', false );
			header( 'Pragma: no-cache' );
			header("Location: acp_flow_stream.php?id=$id");
			exit;
		}
	}
}
...
elseif ($_POST['del_os_policy']) {
	...
	if (isset($id) && $a_nat[$id]) {
		$a_nat[$id] = $natent;
		write_config("acp pkg: deleted a Host_OS_Policy engine.");
	}
	header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
	header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
	header( 'Cache-Control: no-store, no-cache, must-revalidate' );
...
elseif ($_POST['save'] || $_POST['apply']) {
	...
	if (!$input_errors) {
		...
		/* then update the acp.conf file for this	*/
		/* interface.									 */
		/**************************************************/
		if (isset($id) && $a_nat[$id]) {
			$a_nat[$id] = $natent;
			...
			$uuid = $a_nat[$id]['uuid'];
			bp_write_report_db("report_0008_acp_fapp_stream", "Active Protection|{$if_friendly}|{$if}{$uuid}");

			write_config("acp pkg: saved flow or stream configuration changes.");
			$rebuild_rules = false;
...
elseif ($_POST['save_import_alias']) {
	...
	if ($_POST['mode'] =='add_edit_os_policy') {
		...
		if (!$input_errors) {
			...
			write_config("acp pkg: saved Host_OS_Policy engine created from a defined firewall alias.");
			$importalias = false;
			$selectalias = false;
			header( 'Expires: Sat, 26 Jul 1997 05:00:00 GMT' );
			header( 'Last-Modified: ' . gmdate( 'D, d M Y H:i:s' ) . ' GMT' );
...
$pglinks = array("./active_protection/ap_services.php", "/active_protection/acp_interfaces.php", "/active_protection/acp_interfaces_edit.php?id={$id}", "@self");
$pgtitle = array(gettext("Active Protection"), gettext("Interfaces"), "Interface Settings", "{$if_friendly} - Flow and Stream Engine");
...
<?php
	if ($importalias) {
		...
		print('<form action="acp_flow_stream.php" method="post" name="iform" id="iform" class="form-horizontal">');
		...
		include("/usr/local/www/active_protection/acp_import_aliases.php");
		print('</form>');

	} elseif ($add_edit_os_policy) {

		$form = new Form(false);
		include("/usr/local/www/active_protection/sacp_os_policy_engine.php");

	} else {
?>

<form action="acp_flow_stream.php" method="post" name="iform" id="iform" class="form-horizontal">
...
))->setHelp('Max memory to be used by stream engine. Default is 131,217,728 bytes (128MB). Sets the maximum amount of memory, in bytes, to be used by the stream engine. This number will likely need to be increased beyond the default value in systems with more than 4 processor cores. If acp fails to start and logs a memory allocation error, increase this value in 4 MB chunks until acp starts successfully.');
$section->addInput(new Form_Input(
...
$section->addInput(new Form_Checkbox(
	...
	'acp will pick up and track sessions mid-stream. Default is Not Checked.',
...
$section->addInput(new Form_Checkbox(
	...
	'acp will track asynchronous one-sided streams. Default is Not Checked.',
...
),true)->setHelp('Sets max number of extra SYN/ACKs acp will queue and delay judgement on while awaiting proper ACK for 3-way handshake. Default is 5.');
$form->add($section);
...
<div class="infoblock">
	<?=print_info_box('<strong>Note:</strong> Please save your settings before you exit. Changes will rebuild the rules file. This may take several seconds. acp must also be restarted to activate any changes made on this screen.', 'info')?>
</div>

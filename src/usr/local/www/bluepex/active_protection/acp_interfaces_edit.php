...
if (is_null($id)) {
		header("Location: /active_protection/acp_interfaces.php");
		exit;
}
...
if (isset($_GET["action"])) {
	...
	foreach($interfaces as $interface_now) {
		if (strlen(array_search($interface_now, $interface_in_use)) == 0) {
			$array_clean[strtolower($interface_now)] = $interface_now; 
		}
	}
...
elseif (isset($id) && !isset($a_rule[$id])) {
	...
	if (count($ifrules) == count($ifaces)) {
		$input_errors[] = gettext("No more available interfaces to configure for ActiveProtection!");
		$interfaces = array();
		$pconfig = array();
	}
}
...
if (empty($pconfig['mixed_mode']))
	$pconfig['mixed_mode'] = 'off';
if (empty($pconfig['scanned_netmap_interfaces']))
	$pconfig['scanned_netmap_interfaces'] = $pconfig['descr'];
...
if (empty($pconfig['eve_log_flow']))
	$pconfig['eve_log_flow'] = "on";
if (empty($pconfig['eve_log_netflow']))
	$pconfig['eve_log_netflow'] = "on";
if (empty($pconfig['eve_log_stats']))
	$pconfig['eve_log_stats'] = "on";
...
if (isset($_POST["save"]) && !$input_errors) {
	...
	if (isset($_POST['interface'])) {
		foreach ($a_rule as $k => $v) {
			if (($v['interface'] == $_POST['interface']) && ($_POST["action"] == "dup")) {
				$input_errors[] = gettext("The '{$_POST['interface']}' interface is already assigned to another ActiveProtection instance.");
				break;
			}
		}
		//if ((strtolower($_POST['interface']) == 'lan') && ($_POST["action"] == "dup")) {
		//	$show_interface = strtoupper($_POST['interface']);
		//	$input_errors[] = gettext("Don't choose interface $show_interface to use ActiveProtection!");
		//}
		...
		//If duplicate, add new interface tmp
		if ($_POST["action"] == "dup") {
	...
	if ($_POST['enable'] != 'on') {
		...
		$uuid = $a_rule[$id]['uuid'];
		if (isset($a_rule[$id]['mixed_mode']) && $a_rule[$id]['mixed_mode'] == "on") {
			mwexec("pkill -9 -af suricata_{$uuid}_{$if_real}");
		}
		file_put_contents("/etc/suricata_{$if_real}{$uuid}_stop.lck", "");
		file_put_contents("{$g['varrun_path']}/suricata_{$if_real}{$uuid}_stop.lck", "");
		write_config("Active Protection pkg: disabled ActiveProtection on " . convert_friendly_interface_to_friendly_descr($a_rule[$id]['interface']));
		$rebuild_rules = false;
	...
	if (!empty($write_status_log)) {
		...
		$if_real = get_real_interface($a_rule[$id]['interface']);
		$uuid = $a_rule[$id]['uuid'];
		bp_write_report_db($write_status_log, "Active Protection|{$if_friendly}|{$if_real}{$uuid}");
	}
	//else {
	//	header("Location: /active_protection/acp_interfaces.php");
	//}
	...
	$if_msg_extra = false;
	if (isset($_POST['scanned_netmap_interfaces']) && count($_POST['scanned_netmap_interfaces']) > 0) {
		foreach ($config['installedpackages']['suricata']['rule'] as $key => $suricatacfg) {
			foreach($_POST['scanned_netmap_interfaces'] as $interface_now) {
				if (get_real_interface($suricatacfg['interface']) == get_real_interface($interface_now) && get_real_interface($suricatacfg['interface']) != get_real_interface($_POST['interface'])) {
					$interface_now = strtoupper($interface_now);
					if ($suricatacfg['ips_mode'] == "ips_mode_inline") {
						$input_errors[] = gettext("Cannot add interface {$interface_now} because it has Heuristic mode enabled.");
						$if_msg_extra = true;
					}
					if ($suricatacfg['mixed_mode'] == "on") {
						$input_errors[] = gettext("It is not possible to add the {$interface_now} interface for reasons that it is with the extra protection mode activated.");
						$if_msg_extra = true;
					}
				}
			}
	...
	if ($_POST["action"] == "dup") {
		foreach ($config['installedpackages']['suricata']['rule'] as $key => $suricatacfg) {
			if (isset($suricatacfg["scanned_netmap_interfaces"]) && strlen($suricatacfg["scanned_netmap_interfaces"]) > 0 && get_real_interface($suricatacfg['interface']) != get_real_interface($_POST['interface'])) {
				foreach(array_filter(array_unique(explode(",", $suricatacfg["scanned_netmap_interfaces"]))) as $interface_now) {
					if (get_real_interface($interface_now) == get_real_interface($_POST['interface'])) {
						$interface_now = strtoupper($interface_now);
						if ($suricatacfg['ips_mode'] == "ips_mode_inline") {
							$input_errors[] = gettext("Cannot add interface {$interface_now} because it has Heuristic mode enabled.");
							$if_msg_extra = true;
	...
	if ($if_msg_extra) {
		$input_errors[] = gettext("Interface is selected in extra protection operating mode in another interface, remove it from the selection to be able to activate the Heuristic / Extra-Protection operating mode.");
	}
	...
	if (!$input_errors) {
		...
		$old_ips_mode_suricata = $natent['ips_mode'];
		...
		if ($_POST['enable_file_store'] == "on") { $natent['enable_file_store'] = 'on'; }else{ $natent['enable_file_store'] = 'on'; }
		if ($_POST['enable_eve_log'] == "on") { $natent['enable_eve_log'] = 'on'; }else{ $natent['enable_eve_log'] = 'on'; }
		...
		//Mixed mode		
		if ($_POST['mixedMode'] == "on") {
			$natent['ips_mode'] = "ips_mode_legacy";
			$natent['mixed_mode'] = "on";
		} elseif (empty($_POST['mixedMode'])) {
			if ($_POST['ips_mode']) $natent['ips_mode'] = $_POST['ips_mode']; else unset($natent['ips_mode']);
			$natent['mixed_mode'] = "";
		}

		if (isset($_POST['scanned_netmap_interfaces']) && count($_POST['scanned_netmap_interfaces']) > 0) {
			$natent['scanned_netmap_interfaces'] = implode(",",$_POST['scanned_netmap_interfaces']);
		} else {
			$natent['scanned_netmap_interfaces'] = "";
		}
		...
		if ($_POST['eve_redis_key']) {
			$real_interface = get_real_interface($natent['interface']);
			$real_uuid = $natent['uuid'];
			$eve_redis_keys = 'suricata' . $real_interface . $real_uuid;
			$natent['eve_redis_key'] = $eve_redis_keys;
			if (file_exists("/usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml")) {
				$key_identified = mwexec("grep 'key:' /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml | awf -F'\"' '{print $2}'");
				if (intval(trim($key_identified)) != $eve_redis_keys) {
					$lineTarget = intval(trim(mwexec("grep -n 'key:' /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml -n | awk -F\":\" '{print $1}'")));
					exec("sed '{$lineTarget}s/key: \"$key_identified\"/key: \"$eve_redis_keys\"/g' /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml > /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml.tmp");
					exec("mv /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml.tmp /usr/local/etc/suricata/suricata_{$real_uuid}_{$real_interface}/suricata.yaml");		
				}
			}
		...
		if (isset($_REQUEST['id']) &&
		    ...
		    $_REQUEST['action'] != 'dup') {
			bp_write_report_db("report_0008_acp_fapp_edit", "Active Protection|{$if_friendly}|{$if_real}{$uuid}");
		}
		...
		if (isset($_REQUEST['action']) &&
			...
			bp_write_report_db("report_0008_acp_fapp_dup", "Active Protection|{$dup_if_friendly}|{$dup_if_real}{$dup_uuid}|{$if_friendly}|{$if_real}{$uuid}");
		}

		// Save configuration changes
		write_config("ActiveProtection pkg: modified interface configuration for " . convert_friendly_interface_to_friendly_descr($natent['interface']));

		//Reload interface if is running
				...
				if ($id == $key) {
					...
					if (suricata_is_running($uuid, $if)) {
						...
						if (in_array($if, $all_gtw,true)) {
							...
							$map_rule = &$config['installedpackages']['suricata']['rule'][$key];
							$rules_map = suricata_load_rules_map($rulefile);
							suricata_modify_sids_action($rules_map, $map_rule);
						
							$rebuild_rules = true;
							suricata_generate_yaml($map_rule);
							$rebuild_rules = false;
				
							suricata_reload_config($map_rule);
...
$pgtitle = array(gettext("Active Protection"), gettext("Interfaces"), gettext("Edit Interface Settings - ") . $if_friendly);
...
if ($pconfig['enable'] == 'on' && $pconfig['ips_mode'] == 'ips_mode_inline' && (!isset($config['system']['disablechecksumoffloading']) || !isset($config['system']['disablesegmentationoffloading']) || !isset($config['system']['disablelargereceiveoffloading']))) {
	print_info_box(gettext('Inline mode requires that Hardware Checksum, Hardware TCP Segmentation and Hardware Large Receive Offloading ' .
				'all be disabled on the ') . '<b>' . gettext('System > Advanced > Networking') . ' </b>' . gettext('tab.'));
}

$tab_array = array();
$tab_array[] = array(gettext("Interfaces"), true, "/active_protection/acp_interfaces.php");
display_top_tabs($tab_array, true);

$tab_array = array();
$menu_iface=($if_friendly?substr($if_friendly,0,5)." ":"Iface ");
$tab_array[] = array($menu_iface . gettext("Settings"), true, "/active_protection/acp_interfaces_edit.php?id={$id}");
display_top_tabs($tab_array, true);
?>
<div style="width: 100%;text-align:right;">
<img src="../images/bp-gear.png" style="width: 32px" onclick="mostrarModalProcessos()" id="estadoCampoSerial">
</div>
<?
$form = new Form;
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will send Alerts from this interface to the firewall\'s system log.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will periodically log statistics for the interface. Default is Not Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will append-to instead of clearing statistics log file when restarting. Default is Not Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log decoded HTTP traffic for the interface. Default is Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will append-to instead of clearing HTTP log file when restarting. Default is Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log extended HTTP information. Default is Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log TLS handshake traffic for the interface. Default is Not Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log and store TLS certificates for the interface. Default is Not Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('FirewaActiveProtectionllapp will log extended TLS info such as fingerprint. Default is Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log tracked files in JavaScript Object Notation (JSON) format. Default is Not Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will append-to instead of clearing Tracked Files log file when restarting. Default is Checked.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will force logging magic on all logged Tracked Files. Default is Not Checked.'),
...
$section->addInput(new Form_Select(
	...
	array("none" => "None", "md5" => "MD5", "sha1" => "SHA1", "sha256" => "SHA256", "MD5, SHA1, SHA256" => "MD5, SHA1, SHA256")
))->setHelp(gettext('ActiveProtection will generate checksums for all logged Tracked Files using the chosen algorithm. Default is None.'));
$section->addInput(new Form_Checkbox(
	'enable_file_store',
	gettext('Enable File-Store'),
	gettext('ActiveProtection will extract and store files from application layer streams. Default is Not Checked. Warning: This will consume a significant amount of disk space on a busy network when enabled.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will log decoded packets for the interface in pcap-format. Default is Not Checked. This can consume a significant amount of disk space when enabled.'),
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will output selected info in JSON format to a single file or to syslog. Default is Not Checked.'),
...
),true)->setHelp(gettext('Select EVE log output destination. Choosing FILE is suggested, and is the default value.'));
...
$section->addInput(new Form_Select(
	...
	array(  "auth" => "AUTH", "authpriv" => "AUTHPRIV", "daemon" => "DAEMON", "kern" => "KERN", "security" => "SECURITY",
		"syslog" => "SYSLOG", "user" => "USER", "local0" => "LOCAL0", "local1" => "LOCAL1", "local2" => "LOCAL2",
		"local3" => "LOCAL3", "local4" => "LOCAL4", "local5" => "LOCAL5", "local6" => "LOCAL6", "local7" => "LOCAL7" )
),true)->setHelp(gettext('Select EVE syslog output facility.'));
...
),true)->setHelp(gettext('Enter the Redis server IP'));
...
),true)->setHelp(gettext('Enter the Redis server port'));
...
),true)->setHelp(gettext('Select the REDIS output mode'));
...
),true)->setHelp(gettext('Enter the REDIS Key'));
...
),true)->setHelp(gettext('Select HTTP X-Forwarded-For Operation Mode. Extra-Data adds an extra field while Overwrite overwrites the existing source or destination IP. Default is extra-data.'));
...
),true)->setHelp(gettext('Select HTTP X-Forwarded-For Deployment.  Reverse deployment uses the last IP address while Forward uses the first one. Default is reverse.'));
...
),true)->setHelp(gettext('Enter header where actual IP address is reported. Default is X-Forwarded-For. If more than one IP address is present, the last one will be used.'));

$section->addInput(new Form_Checkbox(
	'eve_log_alerts',
	gettext('EVE Log Alerts'),
	gettext('ActiveProtection will output Alerts via EVE'),
...
),true)->setHelp(gettext('Log the payload data with alerts.  Options are No (disable payload logging), Only Printable (lossy) format, Only Base64 encoded or Both. See Suricata documentation.'));
...
$group->setHelp(gettext('Select which details ActiveProtection will use to enrich alerts.'));

$section->add($group)->addClass('eve_log_alerts_details');

$group = new Form_Group(gettext('Enable all EVE log management fields'));

$group->add(new Form_Checkbox(
	'enable_all_options_eve',
	gettext('Enable'),
	gettext('Enable'),
	false,
	'on'
...
$group->setHelp(gettext('Enable all log management fields below.'));
...
$group->add(new Form_Checkbox(
	'eve_log_stats',
	gettext('ActiveProtection Stats'),
	gettext('ActiveProtection Stats'),
...
$group = new Form_Group(gettext('EVE Logged Extended'),true);
...
),true)->setHelp(gettext('ActiveProtection will generate checksums for all logged Tracked Files using the chosen algorithm. Default is None.'));

$group = new Form_Group(gettext('EVE Logged Stats'));
...
$section = new Form_Section(gettext('ActiveProtection Execution and Performance'));
...
if ($pconfig['mixed_mode'] != 'on') {
	$group = new Form_Group(gettext('Active Protection execution'));
	...
	$group->add(new Form_Select(
		...
		gettext('IPS Mode'),
		$pconfig['ips_mode'],
		array("ips_mode_legacy" => gettext('Heuristic') )
	))->setHelp("<p style='color:red;'>Atenção: o modo de proteção extra está ativo, com isso, o campo de seleção de serviços está limitado a somente o modo heurístico, para retomar ao modo normal, desative a proteção extra.</p>");
	$section->add($group);	
}

$section->addInput(new Form_Checkbox(
	'mixedMode',
	gettext('Extra Protection') . " (Inter-Networks)",
	gettext('Activate this field to activate extra protection on the interface.'),
	$pconfig['mixed_mode'] == 'on' ? true:false,
	'on'
))->setHelp("<p>When this option is activated, the interface will operate only in heuristic mode.</p><p style='color:red;'>This option is intended for interfaces that will operate with internet traffic, it is recommended its activation for this use.</p><p style='color:red;'>This option is only available for Active Protection, if the interface becomes an 'exception interface', this option is removed of the interface, if it returns to working as Active Proteciton, it is necessary to reactivate this option.</p>");

$if_real_interface = get_real_interface($pconfig['interface']);
$suricata_uuid = $pconfig['uuid'];

$interfaces_netmap_list = [];
if (isset($pconfig['scanned_netmap_interfaces']) && strlen($pconfig['scanned_netmap_interfaces']) > 0) {
	foreach(explode(",", $pconfig['scanned_netmap_interfaces']) as $line_now) {
		$interfaces_netmap_list[$line_now] = $line_now;
	}
}

$section->addInput(new Form_Select(
	'scanned_netmap_interfaces',
	gettext('Scanner interfaces') . " (Inter-Networks)",
	$interfaces_netmap_list,
	get_configured_interface_with_descr(),
	true
))->setHelp(gettext("<p>Enter the real interfaces to be scanned by the extra protection service.</p><p style='color:red'>NOTE:The real interface is always a checked option and the input of other interfaces operating in Heuristic and/or Extra Protection mode.</p>"));
...
$section->addInput(new Form_Input(
	...
	$pconfig['max_pending_packets']
			...
			gettext('Enter number of simultaneous packets to process. Default is 1024.<br/>This controls the number simultaneous packets the engine can handle. ') .
...
))->setHelp(gettext('Choose a multi-pattern matcher (MPM) algorithm. Auto is the default, and is the best choice for almost all systems.  Auto will use hyperscan if available.'));
...
))->setHelp(gettext('Choose a Signature Group Header multi-pattern matcher context. Default is Auto.<br />AUTO means ActiveProtection selects between Full and Single based on the MPM algorithm chosen. ') .
...
))->setHelp(gettext('Enter limit for recursive calls in content inspection code. Default is 3000.<br />When set to 0 an internal default is used. When left blank there is no recursion limit.'));
...
$section->addInput(new Form_Checkbox(
	...
	gettext('ActiveProtection will place the monitored interface in promiscuous mode when checked.'),
...
))->setHelp(gettext('Enter value in bytes for the interface PCAP snaplen. Default is 1518.  This parameter is only valid when IDS or Legacy Mode IPS is enabled.<br />This value may need to be increased if the physical interface is passing VLAN traffic and expected alerts are not being received.'));

$form->add($section);

$section = new Form_Section(gettext('Networks ActiveProtection Should Inspect and Protect'),'',0,true);
...
$group->setHelp(gettext('The default Pass List adds Gateways, DNS servers, locally-attached networks, ' . 
						'the WAN IP, VPNs and VIPs. Create a Pass List with an alias to customize whitelisted IP addresses.  ' . 
...
$section = new Form_Section(gettext('Arguments here will be automatically inserted into the ActiveProtection configuration'),'',0,true);
$section->addInput(new Form_Textarea (
	'configpassthru',
	gettext('Advanced Configuration Pass-Through'),
	($_POST['configpassthru']?$_POST['configpassthru']:$pconfig['configpassthru'])
))->setHelp(gettext('Enter any additional configuration parameters to add to the ActiveProtection configuration here, separated by a newline'));
...
<div class="infoblock">
	<?=print_info_box(gettext('<strong>Note:</strong> Please save your settings before you attempt to start ActiveProtection.'), gettext('info'))?>
</div>
...
events.push(function(){
	...
	$('#enable_all_options_eve').click(function() {
		if ($("#enable_all_options_eve").prop('checked')) {
			$("#eve_log_http").prop('checked', true);
			$("#eve_log_dns").prop('checked', true);
			$("#eve_log_smtp").prop('checked', true);
			$("#eve_log_nfs").prop('checked', true);
			$("#eve_log_smb").prop('checked', true);
			$("#eve_log_krb5").prop('checked', true);
			$("#eve_log_ikev2").prop('checked', true);
			$("#eve_log_tftp").prop('checked', true);
			$("#eve_log_tls").prop('checked', true);
			$("#eve_log_ssh").prop('checked', true);
			$("#eve_log_dhcp").prop('checked', true);
			$("#eve_log_files").prop('checked', true);
			$("#eve_log_stats").prop('checked', true);
			$("#eve_log_flow").prop('checked', true);
			$("#eve_log_netflow").prop('checked', true);
		}
		status_enable_all_eve();
	});

	function status_enable_all_eve() {
		if (
			$("#eve_log_http").prop('checked') &&
			$("#eve_log_dns").prop('checked') &&
			$("#eve_log_smtp").prop('checked') &&
			$("#eve_log_nfs").prop('checked') &&
			$("#eve_log_smb").prop('checked') &&
			$("#eve_log_krb5").prop('checked') &&
			$("#eve_log_ikev2").prop('checked') &&
			$("#eve_log_tftp").prop('checked') &&
			$("#eve_log_tls").prop('checked') &&
			$("#eve_log_ssh").prop('checked') &&
			$("#eve_log_dhcp").prop('checked') &&
			$("#eve_log_files").prop('checked') &&
			$("#eve_log_stats").prop('checked') &&
			$("#eve_log_flow").prop('checked') &&
			$("#eve_log_netflow").prop('checked')
		) {
			$("#enable_all_options_eve").prop('checked', true);
		} else {
			$("#enable_all_options_eve").prop('checked', false);
		}
	}

	status_enable_all_eve();

	$('#eve_log_netflow').click(function() { status_enable_all_eve(); });
	$('#eve_log_flow').click(function() { status_enable_all_eve(); });
	$('#eve_log_stats').click(function() { status_enable_all_eve(); });
	$('#eve_log_files').click(function() { status_enable_all_eve(); });
	$('#eve_log_dhcp').click(function() { status_enable_all_eve(); });
	$('#eve_log_ssh').click(function() { status_enable_all_eve(); });
	$('#eve_log_tls').click(function() { status_enable_all_eve(); });
	$('#eve_log_tftp').click(function() { status_enable_all_eve(); });
	$('#eve_log_ikev2').click(function() { status_enable_all_eve(); });
	$('#eve_log_krb5').click(function() { status_enable_all_eve(); });
	$('#eve_log_smb').click(function() { status_enable_all_eve(); });
	$('#eve_log_nfs').click(function() { status_enable_all_eve(); });
	$('#eve_log_smtp').click(function() { status_enable_all_eve(); });
	$('#eve_log_dns').click(function() { status_enable_all_eve(); });
	$('#eve_log_http').click(function() { status_enable_all_eve(); });
	...
	function enable_realInterfacesInterNetworksScan() {		
		if ($('#ips_mode').val() == 'ips_mode_legacy') {
			hideSelect('blockoffendersip', true);
		} else {
	...
	$('#mixedMode').click(function() {
		enable_realInterfacesInterNetworksScan();
	});
...
<div class="modal fade" id="mostrarModalFields" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="TituloModalCentralizado"><?=gettext("Release log field's")?> </h5>
			...
			<div class="modal-body">
				...
				<button type="submit" name="confirmar_senha_liberar_campos" id="confirmar_senha_liberar_campos" class="btn btn-primary"><?=gettext("Liberar")?></button>
			</div>
		</div>
	</div>
</div>
...
$(document).ready(function() {
	...
	$("#confirmar_senha_liberar_campos").click(function (event) {
		$.ajax({
		...
		success: function(data) {
			if (data == "true") {
				$('#mostrarModalFields').modal('hide');
				hideSelect('detect_eng_profile',false);
				hideSelect('mpm_algo',false);
				hideSelect('sgh_mpm_context',false);
				hideCheckbox('delayed_detect',false);
				if (document.getElementById('estadoCampoSerial').src.split("/")[document.getElementById('estadoCampoSerial').src.split("/").length-1] == 'bp-gear.png') {
					document.getElementById('estadoCampoSerial').src="../images/bp-logout.png";
				}
			} else {
				alert("<?=gettext("Incorrect password!")?>");
				document.getElementById('senhaLiberar').value="";
			}
		}
    	});
	});
...
function mostrarModalProcessos() {
	$.ajax({
		...
		success: function(data) {
			if (data == "true") {
				document.getElementById('estadoCampoSerial').src="../images/bp-gear.png";
				document.getElementById('senhaLiberar').value="";
				hideSelect('detect_eng_profile',true);
				hideSelect('mpm_algo',true);
				hideSelect('sgh_mpm_context',true);
				hideCheckbox('delayed_detect',true);
			} else {
				$('#mostrarModalFields').modal('show');
			}
		}
	});
}
...
<?php if ($pconfig['mixed_mode'] == 'on'): ?>
	hideCheckbox('blockoffenderskill',true);
	hideCheckbox('intf_promisc_mode',true);
<?php endif; ?>

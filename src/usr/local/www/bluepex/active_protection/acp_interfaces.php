...
if (isset($_POST['del_x'])) {
	...
	if (isset($config['captiveportal']['ActiveProtection_lan'])) {
		log_error("Unable to delete instance ActiveProtection_lan, it is being used by ActiveProtection by users.");
	...
	if (is_array($_POST['rule']) && count($_POST['rule'])) {
		foreach ($_POST['rule'] as $rulei) {
			...
			if (isset($a_nat[$rulei]["mixed_mode"]) && $a_nat[$rulei]["mixed_mode"] == "on") {
				mwexec("kill -9 `ps ax | grep suricata_{$suricata_uuid}_{$if_real} | grep 'suricata2.yaml' | grep -v grep | awk -F\" \" '{print $1}'`");
			}
			...
			suricata_clean_destroy_tables_pfctl($suricata_uuid, $if_real, 'acp2c');		
			bp_write_report_db("report_0008_acp_fapp_removed", "Active Protection|{$if_friendly}|{$if_real}{$suricata_uuid}");
			unset($a_nat[$rulei]);
		}

		/* If all the ActiveProtection interfaces are removed, then unset the config array. */
		unset($a_nat);
		write_config("ActiveProtection pkg: deleted one or more ActiveProtection interfaces.");
		...
		file_put_contents('/tmp/interfaces_suricata_acp', getInterfaceNewAcp());
	...
	if (is_numeric($delbtn_list) && $a_nat[$delbtn_list]) {
		...
		$suricata_eve_redis = $a_nat[$delbtn_list]['eve_redis_key'];
		log_error("Stopping ActiveProtection on {$if_friendly}({$if_real}) due to interface deletion...");
		...
		rmdir_recursive("{$suricatalogdir}suricata_{$if_real}{$suricata_uuid}");
		rmdir_recursive("{$suricatadir}suricata_{$suricata_uuid}_{$if_real}");
		if (isset($a_nat[$delbtn_list]['mixed_mode']) && $a_nat[$delbtn_list]['mixed_mode'] = 'on') {
			if (intval(trim(shell_exec("ps ax | grep 'suricata_{$if_real}{$suricata_uuid}' | grep -v grep -c"))) > 0) {
				mwexec("/bin/pkill -9 -af suricata_{$if_real}{$suricata_uuid}");
			}
		}
		mwexec("/usr/local/bin/redis-cli del {$suricata_eve_redis}");
		...
		log_error("Deleted ActiveProtection instance on {$if_friendly}({$if_real}) per user request...");
...
if ($_POST['by2toggle']) {
	...
	if (!suricata_is_running($suricatacfg['uuid'], $if_real, 'barnyard2')) {
		// No need to rebuild ActiveProtection rules for Barnyard2,
	...
	log_error("Toggle (barnyard starting) for {$desc_log}...");
}

/* start/stop ActiveProtection */
if ($_POST['toggle']) {
	...
	// ActiveProtection can take several seconds to startup, so to
	...
	// Create steps for the background task to start ActiveProtection.
	...
	switch ($_POST['toggle']) {
		case 'start':
			...
			copy("/usr/local/pkg/suricata/yalm/acp/suricata_yaml_template.inc", "/usr/local/pkg/suricata/suricata_yaml_template.inc");
			...
			} elseif ($tw_includes1 == "only") {
				foreach (glob("/usr/local/share/suricata/rules/*") as $files) {
					unlink_if_exists($files);
				}
				copy("/usr/local/share/suricata/rules_fapp/_ameacas.rules", "/usr/local/share/suricata/rules/_ameacas.rules");
				copy("/usr/local/share/suricata/rules_fapp/_ameacas_ext.rules", "/usr/local/share/suricata/rules/_ameacas_ext.rules");
				copy("/usr/local/share/suricata/rules_fapp/_emerging.rules", "/usr/local/share/suricata/rules/_emerging.rules");
				copy("/usr/local/share/suricata/rules_fapp/reputation.rules", "/usr/local/share/suricata/rules/reputation.rules");
			}

			$text_log_start = 'report_0008_acp_fapp_start';

			if (suricata_is_running($suricatacfg['uuid'], $if_real)) {
				log_error("Restarting ActiveProtection on {$if_friendly}({$if_real}) per user request...");
			...
			foreach ($config['installedpackages']['suricata']['rule'] as $key => $suricatacfg) {
				...
				if ($suricatacfg['interface'] == $if_real ||
				    ...
				    file_exists("/etc/suricata_{$if}{$uuid}_mix_netmap_on")) {
					continue;
				}
				unlink_if_exists("{$g['varrun_path']}/suricata_{$if}{$uuid}.pid");
				unlink_if_exists("{$g['varrun_path']}/suricata_{$if}{$suricatacfg['uuid']}_stop.lck");
				unlink_if_exists("/etc/suricata_{$if}{$suricatacfg['uuid']}_stop.lck");

				if ($suricatacfg['mixed_mode'] == 'on') {
					foreach (glob("/usr/local/share/suricata/rules/*") as $files) {
						unlink_if_exists($files);
					}
					copy("/usr/local/share/suricata/acp_mix/suricata2.rules", "/usr/local/etc/suricata/suricata_{$uuid}_{$if}/rules/suricata2.rules");
					copy("/usr/local/share/suricata/rules_acp/_emerging.rules", "/usr/local/share/suricata/rules/_emerging.rules");
				}
				...
				foreach (glob("/usr/local/share/suricata/otx/ransomd5/*") as $files) {
					copy($files, "/usr/local/etc/suricata/suricata_{$uuid}_{$if}/rules/".basename($files));
				}
				...
				sleep(2);
				log_error("Starting ActiveProtection on {$if_friendly}({$if_real}) per user request...");
				suricata_start($suricatacfg, $if_real);
				bp_write_report_db($text_log_start, "Active Protection|{$if_friendly}|{$if_real}{$suricata_uuid}");
				sleep(28);
				break;
			}
			bp_cron_acp_fapp_action();
		...
		case 'stop':
			...
			if (suricata_is_running($suricata_uuid, $if_real)) {
				log_error("Stopping ActiveProtection on {$if_friendly}({$if_real}) per user request...");
				unlink_if_exists("{$g['varrun_path']}/suricata_start_all.lck");
				bp_write_report_db("report_0008_acp_fapp_stop", "Active Protection|{$if_friendly}|{$if_real}{$suricata_uuid}");
				suricata_stop($suricatacfg, $if_real);
			}
			...
			if ($suricatacfg['mixed_mode'] == 'on') {
				mwexec("/sbin/pkill -9 -af suricata_{$if_real}{$suricata_uuid}");
...
/* Ajax call to periodically check ActiveProtection status */
/* on each configured interface.                   */
if ($_POST['status'] == 'check') {
	$list = array();

	// Iterate configured ActiveProtection interfaces and get status of each
...
$pgtitle = array(gettext("Active Protection"), gettext("Interfaces"));
...
<div style='margin: auto; text-align:end;'>
	<a href="./acp_global_config.php"><img style='width: 32px;' src='../images/bp-gear.png' id="changePriority"></a>
</div>

<form action="acp_interfaces.php" method="post" enctype="multipart/form-data" name="iform" id="iform">
			...
			<table id="maintable" class="table table-striped table-hover table-condensed">
				<thead>
					<tr id="frheader">
						...
						<th><?=gettext("ActiveProtection"); ?></th>
				...
				<tbody>
					...
					?>
						...
						<?php if ($check_suricata_info == "on") : ?>
							<?php if (suricata_is_running($suricata_uuid, $if_real)) : ?>
								<i id="suricata_<?=$if_real.$suricata_uuid;?>" class="fa fa-check-circle text-success icon-primary" title="<?=gettext('ActiveProtection is running on this interface');?>"></i>
								&nbsp;
								<i name="click_ativa3" class="fa fa-play-circle icon-pointer icon-primary text-info hidden" onclick="javascript:suricata_iface_toggle('start', '<?=$nnats?>');" title="<?=gettext('Start ActiveProtection on this interface');?>"></i>
								<i name="click_disabled3" id="suricata_<?=$if_real.$suricata_uuid;?>_stop" class="fa fa-stop-circle-o icon-pointer icon-primary text-info" onclick="javascript:suricata_iface_toggle('stop', '<?=$nnats?>');" title="<?=gettext('Stop ActiveProtection on this interface');?>"></i>
							<?php elseif ($suri_starting[$nnats] == 'TRUE' || file_exists("{$g['varrun_path']}/suricata_pkg_starting.lck")) : ?>
								<i id="suricata_<?=$if_real.$suricata_uuid;?>" class="fa fa-cog fa-spin text-success icon-primary" title="<?=gettext('ActiveProtection is starting on this interface');?>"></i>
								&nbsp;
								<i name="click_ativa3" class="fa fa-play-circle icon-pointer icon-primary text-info hidden" onclick="javascript:suricata_iface_toggle('start', '<?=$nnats?>');" title="<?=gettext('Start ActiveProtection on this interface');?>"></i>
								<i name="click_disabled3" id="suricata_<?=$if_real.$suricata_uuid;?>_stop" class="fa fa-stop-circle-o icon-pointer icon-primary text-info" onclick="javascript:suricata_iface_toggle('stop', '<?=$nnats?>');" title="<?=gettext('Stop ActiveProtection on this interface');?>"></i>
							<?php else: ?>
								<i class="fa fa-times-circle text-danger icon-primary" title="<?=gettext('ActiveProtection is stopped on this interface');?>"></i>
								&nbsp;
								<i name="click_ativa3" class="fa fa-play-circle icon-pointer icon-primary text-info" onclick="javascript:suricata_iface_toggle('start', '<?=$nnats?>');" title="<?=gettext('Start ActiveProtection on this interface');?>"></i>
								<i name="click_disabled3" id="suricata_<?=$if_real.$suricata_uuid;?>_stop" class="fa fa-stop-circle-o icon-pointer icon-primary text-info hidden" onclick="javascript:suricata_iface_toggle('stop', '<?=$nnats?>');" title="<?=gettext('Stop ActiveProtection on this interface');?>"></i>
							<?php endif; ?>
						<?php else : ?>
							<?=gettext('DISABLED');?>&nbsp;
						<?php endif; ?>
						...
						<td>
							<?php
							$text_mode = "Heuristic";
							if (isset($natent['mixed_mode']) && $natent['mixed_mode'] == "on") {
								$text_mode .= " (Extra Protection)";
							} elseif ($natent['ips_mode'] == "ips_mode_legacy") {
						...
						<td>
							<a href="acp_interfaces_edit.php?id=<?=$nnats;?>" class="fa fa-pencil icon-primary" title="<?=gettext('Edit this ActiveProtection interface mapping');?>"></a>
							<?php if ($id_gen < count($ifaces)): ?>
								<a href="acp_interfaces_edit.php?id=<?=$nnats?>&action=dup" class="fa fa-clone" title="<?=gettext('Clone this ActiveProtection instance to an available interface');?>"></a>
							<?php endif; ?>
							<a href="acp_flow_stream.php?id=<?=$nnats?>" class="fa fa-cog" title="<?=gettext('Advanced Config to Memory Flow');?>"></a>
							<a style="cursor:pointer;" class="fa fa-trash no-confirm icon-primary" id="Xldel_<?=$nnats?>" title="<?=gettext('Delete this ActiveProtection interface mapping'); ?>"></a>
							<button style="display: none;" class="btn btn-xs btn-warning" type="submit" id="ldel_<?=$nnats?>" name="ldel_<?=$nnats?>" value="ldel_<?=$nnats?>" title="<?=gettext('Delete this ActiveProtection interface mapping'); ?>"><?=gettext("Delete this ActiveProtection interface mapping")?></button>
							<i class="fa fa-info-circle icon-pointer" data-placement="right" style="color: #337AB7; font-size:20px; margin-bottom: 10px;" title="<?="Interface: " . $natent['interface'] . "&#013;Interface real: " . get_real_interface($natent['interface']) ."&#013;Identificação: " . $natent['uuid']?>"></i>
						</td>
					...
					<tr>
						<td></td>
						<td colspan="7">
							<?php if ($no_rules_footnote): ?><span class="text-danger"><?=gettext("WARNING: Marked interface currently has no rules defined for ActiveProtection"); ?></span>
							<?php endif; ?>
						</td>
					</tr>
				</tbody>
...
<div class="infoblock">
	...
	<div class="row">
		...
		<div class="col-md-6">
			<p>
				<i class="fa fa-lg fa-check-circle" alt="Running"></i> <i class="fa fa-lg fa-times" alt="Not Running"></i> ' . gettext("Display the current status of ActiveProtection and") . '<br/>
...
//<![CDATA[
	...
	function showStatus(responseData) {
		...
		for(var key in data) {
			var service_name = 'ActiveProtection';
	...
	function intf_del() {
		...
		if (isSelected)
			return confirm('Do you really want to delete the selected ActiveProtection mapping?');
		else
			alert("There is no ActiveProtection mapping selected for deletion.  Click the checkbox beside the ActiveProtection mapping(s) you wish to delete.");
	}

	events.push(function() {
		$('[id^=Xldel_]').click(function (event) {
			if (confirm("<?=gettext('Delete this ActiveProtection interface mapping?')?>")) {

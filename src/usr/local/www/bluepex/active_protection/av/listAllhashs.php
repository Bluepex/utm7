...
$pgtitle = array(gettext("Active Protection"), gettext("File Parsing Gateway"), gettext("Status Hash Lists"));
...
<form action="./listAllhashs.php" method="POST" style="display:none;" id="startScanValues">
...
<form action="./listAllhashs.php" method="POST" style="display:none;" id="startScanValuesSimple">
...
<div class="p-0" style="margin-top:10px!important;">
	<div class="col-12 cards-info">
		<div class="row pb-2 pr-xl-7">
			<div class="col-sm card-app p-3 p-2 mb-2" style="height: 100% !important;">
				<h6><?=gettext("Values of Lists HASH's")?></h6>
				<hr>
				<div style="display: flex; margin-bottom:10px;">
					<input type="text" class="form-control" style="background-color: #FFF!important; width:400px; margin-left:auto;" id="search-hash-file" placeholder="<?=gettext("Search for...")?>">	
					<select class='form-control' style="background-color: #FFF!important; width:100px;" id="selectAmountShowTables">
				...
				<div style="display: flex">
				    <div class="panel-body table-responsive" style="background: transparent !important; width:30%;margin:auto;margin-top:0px;height:300px;">
						<table class="table table-striped table-hover table-condensed sortable-theme-bootstrap" style="background: transparent !important;" data-sortable>
							<thead>
							<tr class="sortableHeaderRowIdentifier text-nowrap">
								<th id="hashCountBlack"><?=gettext("Hash's BlackList"); ?></th>
							</tr>
							</thead>
							<tbody id="table-file-rules-lists-blacklist">
								<tr>
									<td>---</td>
								</tr>
							</tbody>
					...
					<div class="panel-body table-responsive" style="background: transparent !important; width:30%;margin:auto;margin-top:0px;height:300px;">
						<table class="table table-striped table-hover table-condensed sortable-theme-bootstrap" style="background: transparent !important;" data-sortable>
							<thead>
							<tr class="sortableHeaderRowIdentifier text-nowrap">
								<th id="hashCountWhite"><?=gettext("Hash's WhiteList"); ?></th>
							</tr>
							</thead>
							<tbody id="table-file-rules-lists-whitelist">
								<tr>
									<td>---</td>
								</tr>
							</tbody>
					...
					<div class="panel-body table-responsive" style="background: transparent !important; width:30%;margin:auto;margin-top:0px;height:300px;">
						<table class="table table-striped table-hover table-condensed sortable-theme-bootstrap" style="background: transparent !important;" data-sortable>
							<thead>
							<tr class="sortableHeaderRowIdentifier text-nowrap">
								<th id="hashCountException"><?=gettext("Hash's Exceptions"); ?></th>
							</tr>
							</thead>
							<tbody id="table-file-rules-lists-exceptions">
								<tr>
									<td>---</td>
								</tr>
							</tbody>
...
<form action="./listAllhashs.php" method="POST" style="display:none;" id="saveTimeStopServices">
...
<form action="./listAllhashs.php" method="POST" style="display:none;" id="saveCleanStopService">
...
function preencherSearchDataFile(entrada) {
	$("#search-hash-file").val(entrada);
	searchDataFile();
}

function clearSearchDataFile() {
	$("#search-hash-file").val("");
	searchDataFile();
}

function searchDataFile() {
	var $rows = $('#table-file-rules-geral tr');
	var val = $.trim($('#search-hash-file').val()).replace(/ +/g, ' ').toLowerCase();
...
function get_rules_type_list() {
	...
	"./ajax_update_screen_clamav.php",
	{
		...
		data: {
			type_list: 'black',
			find_hash: $("#search-hash-file").val(),
			selectAmountShowTables: $("#selectAmountShowTables").val()
		},
	}).done(function(data) {
		$("#table-file-rules-lists-blacklist").html(data);
	});
	$.ajax(
	"./ajax_update_screen_clamav.php",
	{
		...
		$("#table-file-rules-lists-whitelist").html(data);
	});
	$.ajax(
	"./ajax_update_screen_clamav.php",
	{
		type: 'post',
		data: {
			type_list: 'exceptions',
			find_hash: $("#search-hash-file").val(),
			selectAmountShowTables: $("#selectAmountShowTables").val()
		},
	}).done(function(data) {
		$("#table-file-rules-lists-exceptions").html(data);
	});
	setTimeout(() => {
		$("#hashCountBlack").html("<?=gettext("Hash's BlackList")?>: " + $("#table-file-rules-lists-blacklist td").length);
	}, 1000);
	setTimeout(() => {
		$("#hashCountWhite").html("<?=gettext("Hash's WhiteList")?>: " + $("#table-file-rules-lists-whitelist td").length);
	}, 1100);
	setTimeout(() => {
		$("#hashCountExceptions").html("<?=gettext("Hash's Exceptions")?>: " + $("#table-file-rules-lists-exceptions td").length);
	}, 1200);

}

get_rules_type_list()
window.setInterval("get_rules_type_list()", 5000);

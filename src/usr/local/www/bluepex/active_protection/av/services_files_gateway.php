...
$pgtitle = array(gettext("Active Protection"), gettext("File Parsing Gateway"));
...
<form action="./services_files_gateway.php" method="POST" style="display:none;" id="startScanValues">
...
<form action="./services_files_gateway.php" method="POST" style="display:none;" id="startScanValuesSimple">
...
<div class="p-0" style="margin-top:10px!important;">
	<div class="col-12 cards-info">
		<div class="row pb-2 pr-xl-7">
			<div class="col-sm card-app p-3 p-2 mb-2" style="height: 100% !important;">
				<h6><?=gettext("FILE INSPECTION") . " - RISK FILES (SIMPLE)"?></h6>
				<hr>
				<div class="col-12 text-center margins-content-bottom" style="display:flex;">
					<div class="col-12 col-sm" style="margin-top:100px;">
						<img src="../../images/bp-logout.png" style="width:64px;height:64px;">
                        <h4 class="weight-700"><?=gettext("Potentially dangerous files")?></h4>
						<p class="weight-600 color-text-second" id="update_counter_negate_file_simple">0</p>
                    </div>  
                    <div class="panel panel-default" style="background: transparent !important; margin-top:10px; width: 90%;">
                        <div class="panel-heading" style="background: transparent !important; margin-top:10px;">
                            <h2 class="panel-title" style="background: transparent !important; margin-top:unset;"><?=gettext("FILTERED FILES")?></h2>
                        </div>
                        <button type="click" onclick="clearSearchDataFileSimple()" class="btn btn-danger form-control find-values" style="width: auto; float:right;"><i class="fa fa-times"></i> </button>
                        <input type="text" class="form-control" style="background-color: #FFF!important; float:right; width:400px;" id="search-rule-file-simple" onkeydown="searchDataFileSimple()" onkeyup="searchDataFileSimple()" placeholder="<?=gettext("Search for...")?>">	
                        <div class="panel-body table-responsive" style="height: 265px !important;background: transparent !important;">
                            <table class="table table-striped table-hover table-condensed sortable-theme-bootstrap" style="background: transparent !important;" data-sortable>
                                <thead>
                                <tr class="sortableHeaderRowIdentifier text-nowrap">
									<th data-sortable-type="alpha"><?=gettext("Hash SHA256"); ?></th>
									<th data-sortable-type="alpha"><?=gettext("Type Found"); ?></th>
                                </tr>
                                </thead>
                                <tbody id="table-file-rules-geral-simple">
									<tr>
										<td>---</td>
										<td>---</td>
									</tr>
...
<div class="p-0" style="margin-top:10px!important;">
	<div class="col-12 cards-info">
		<div class="row pb-2 pr-xl-7">
			<div class="col-sm card-app p-3 p-2 mb-2" style="height: 100% !important;">
				<h6><?=gettext("FILE INSPECTION") . " - RISK FILES (ADVANCED)"?></h6>
				<hr>
				<div class="col-12 text-center margins-content-bottom" style="display:flex;">
					<div class="col-12 col-sm" style="margin-top:100px;">
						<img src="../../images/bp-logout.png" style="width:64px;height:64px;">
                        <h4 class="weight-700"><?=gettext("Potentially dangerous files")?></h4>
						<p class="weight-600 color-text-second" id="update_counter_negate_file_advanced">0</p>
                    </div>  
                    <div class="panel panel-default" style="background: transparent !important; margin-top:10px; width: 90%;">
                        <div class="panel-heading" style="background: transparent !important; margin-top:10px;">
                            <h2 class="panel-title" style="background: transparent !important; margin-top:unset;"><?=gettext("FILTERED FILES")?></h2>
                        </div>
                        <button type="click" onclick="clearSearchDataFileAdvanced()" class="btn btn-danger form-control find-values" style="width: auto; float:right;"><i class="fa fa-times"></i> </button>
                        <input type="text" class="form-control" style="background-color: #FFF!important; float:right; width:400px;" id="search-rule-file-advanced" onkeydown="searchDataFile()" onkeyup="searchDataFile()" placeholder="<?=gettext("Search for...")?>">	
                        <div class="panel-body table-responsive" style="height: 265px !important;background: transparent !important;">
                            <table class="table table-striped table-hover table-condensed sortable-theme-bootstrap" style="background: transparent !important;" data-sortable>
                                <thead>
                                <tr class="sortableHeaderRowIdentifier text-nowrap">
                                ...
                                <tbody id="table-file-rules-geral-advanced">
									<tr>
										<td>---</td>
										<td>---</td>
									</tr>
...
<form action="./services_files_gateway.php" method="POST" style="display:none;" id="saveTimeStopServices">
...
<form action="./services_files_gateway.php" method="POST" style="display:none;" id="saveCleanStopService">
...
//-------------------------------------------------
//File request All
function preencherSearchDataFileAdvanced(entrada) {
	$("#search-rule-file-advanced").val(entrada);
	SearchDataFileAdvanced();
}

function preencherSearchDataFileSimple(entrada) {
	$("#search-rule-file-simple").val(entrada);
	SearchDataFileSimple();
}

function clearSearchDataFileAdvanced() {
	$("#search-rule-file-advanced").val("");
	SearchDataFileAdvanced();
}

function clearSearchDataFileSimple() {
	$("#search-rule-file-simple").val("");
	SearchDataFileSimple();
}

function SearchDataFileAdvanced() {
	var $rows = $('#table-file-rules-geral-advanced tr');
	var val = $.trim($('#search-rule-file-advanced').val()).replace(/ +/g, ' ').toLowerCase();
...
function SearchDataFileSimple() {
	var $rows = $('#table-file-rules-geral-simple tr');
	var val = $.trim($('#search-rule-file-simple').val()).replace(/ +/g, ' ').toLowerCase();
	$rows.show().filter(function() {
		var text = $(this).text().replace(/\s+/g, ' ').toLowerCase();
		return !~text.indexOf(val);
	}).hide();
...
//-------------------------------------------------
//Ajax request status
function updateScreenClamav() {
	$.ajax(
		"./ajax_update_screen_clamav.php",
		{
			type: 'post',
			data: {
				update_counter_negate_file_simple: true
			},
		}).done(function(data) {
            $("#update_counter_negate_file_simple").html(data);
	...
	$.ajax(
		"./ajax_update_screen_clamav.php",
		{
			type: 'post',
			data: {
				update_counter_negate_file_advanced: true
			},
		}).done(function(data) {
            $("#update_counter_negate_file_advanced").html(data);
	...
	$.ajax(
		"./ajax_update_screen_clamav.php",
		{
			type: 'post',
			data: {
				update_table_files_simple: true
			},
		}).done(function(data) {
            $("#table-file-rules-geral-simple").html(data);
			preencherSearchDataFileSimple($("#search-rule-file-simple").val());
        }
	);
    $.ajax(
		"./ajax_update_screen_clamav.php",
		{
			type: 'post',
			data: {
				update_table_files_advanced: true
			},
		}).done(function(data) {
            $("#table-file-rules-geral-advanced").html(data);
			preencherSearchDataFileAdvanced($("#search-rule-file-advanced").val());
        }
	);
}
updateScreenClamav();
window.setInterval("updateScreenClamav()",5000);
//--------------------------------------------------

...
if (file_exists("/usr/local/www/active_protection/av/YARA/enable_yara_filters")) {
    $filtersEnable = array_filter(array_unique(explode(";",trim(file_get_contents("/usr/local/www/active_protection/av/YARA/enable_yara_filters")))));
} else {
    if (intval(trim(shell_exec("/bin/ls /usr/local/www/active_protection/av/YARA/ | grep -E 'index' -c"))) > 0) {
        $filtersEnable = array_filter(explode("\n", trim(shell_exec("/bin/ls /usr/local/www/active_protection/av/YARA/ | grep -E 'index'"))));
        file_put_contents('/usr/local/www/active_protection/av/YARA/enable_yara_filters', implode(";", $filtersEnable));
    }
}
if (count($filtersEnable) == 0) {
    die;
}
...
if (intval(getInterfaceNewAcp()) == 0) {
    if (file_exists("/etc/monitor_gateway_files_yara")) {
        if (trim(file_get_contents("/etc/monitor_gateway_files_yara")) == "true") {
            file_put_contents("/etc/monitor_gateway_files_yara", "false");
        }
    }
    if (file_exists("/var/log/yara_work.log")) {
        unlink("/var/log/yara_work.log");
...
//Start
if (file_exists("/etc/monitor_gateway_files_yara") && (trim(file_get_contents("/etc/monitor_gateway_files_yara")) == "true")) {
...
if (file_exists('/etc/monitor_gateway_files_yara') && trim(file_get_contents('/etc/monitor_gateway_files_yara')) == "true") {
    ...
    if (file_exists("/tmp/find_threads_use") && strlen(file_get_contents("/tmp/find_threads_use")) > 0) {
        ...
        mwexec("echo '-----------------------------------------------------------------------------\n' >> /var/log/yara_work.log");
        //mwexec("/usr/local/bin/python3.8 /usr/local/www/active_protection/av/YARA/yara_multiprocessing_scanner.py -r -p4 /usr/local/www/active_protection/av/YARA/index.yar . | grep suricata | awk -F\" \" '{ print $2 \": \" $1\"-1 FOUND\"}' >> /var/log/yara_work.log");
        //mwexec("/usr/local/bin/yara /usr/local/www/active_protection/av/YARA/signature_base_index.yar --scan-list /tmp/find_threads_use >> /var/log/yara_work.log");
        //mwexec("/usr/local/bin/yara /usr/local/www/active_protection/av/YARA/malware_index.yar --scan-list /tmp/find_threads_use >> /var/log/yara_work.log");
        //mwexec("/usr/local/bin/yara /usr/local/www/active_protection/av/YARA/exploit_index.yar --scan-list /tmp/find_threads_use >> /var/log/yara_work.log");
        //mwexec("/usr/local/bin/yara /usr/local/www/active_protection/av/YARA/cve_index.yar --scan-list /tmp/find_threads_use >> /var/log/yara_work.log");
        $filtersEnable = array_unique(array_filter($filtersEnable));
        foreach($filtersEnable as $lineFilter) {
            //mwexec("/usr/local/bin/yara --no-warnings --threads=4 /usr/local/www/active_protection/av/YARA/{$lineFilter} --scan-list /tmp/find_threads_use | grep suricata | awk -F\" \" '{ print $2 \": \" $1\"-1 FOUND\"}' >> /var/log/yara_work.log");
            mwexec("/usr/local/bin/yara --no-warnings --threads=4 --fast-scan --max-rules=1 /usr/local/www/active_protection/av/YARA/{$lineFilter} --scan-list /tmp/find_threads_use | grep suricata | awk -F\" \" '{ print $2 \": \" $1\"-1 FOUND\"}' >> /var/log/yara_work.log");
        }
        
        mwexec("/usr/bin/grep -v \"warning: rule\" /var/log/yara_work.log > /var/log/yara_work.log.tmp && mv /var/log/yara_work.log.tmp /var/log/yara_work.log");

        //Get FOUND clamd
        $values_blacklist = [];
        if (file_exists("/var/log/yara_work.log")) {
            foreach (array_filter(explode("\n", shell_exec("grep ' FOUND' /var/log/yara_work.log | awk -F\": \" '{ print $1 }' | grep '/var/log' | grep -v tmp"))) as $line_work) {

...
$pgtitle = array(gettext("Active Protection"), gettext("Ameaças"));
...
$backup = "/usr/local/share/suricata/rules_acp/_ameacas_ext.bk.rules";
...
foreach(file($customizado) as $linhaTratamento) {
	...
	if (!empty($linhaTratamento)) {
		if (substr($linhaTratamento, 0, 1) != "#") {
			...
			if (substr($msg, count($msg)-2, 1) == "'") {
				$msg = substr($msg, 0, count($msg)-2);
			}	
			
			$filtrar = explode(" ", $msg)[0];
			if (strpos("(", $filtrar) >= 0) {
				$filtrar = explode("(", $filtrar)[0];
				$filtrar = explode("-", $filtrar)[0] . "-" . explode("-", $filtrar)[1] . "-" . explode("-", $filtrar)[2];
			}

			$liberarPUSH = false;
			if (isset($_POST['pesquisarCategoria'])) {
				if ((is_numeric($_POST['pesquisarCategoria']) && $_POST['pesquisarCategoria'] == trim($sid)) ||
					#($_POST['pesquisarCategoria'] == $filtrar)) {
					(explode("-", $_POST['pesquisarCategoria'])[0] . "-" . explode("-", $_POST['pesquisarCategoria'])[1] . "-" . explode("-", $_POST['pesquisarCategoria'])[2] == $filtrar)) {
						$liberarPUSH = true;
					}
				...
				if ((is_numeric($_POST['search-rules-acp-categorias-advanced']) && $_POST['search-rules-acp-categorias-advanced'] == $sid) ||
					(strpos(strtolower($filtrar), strtolower($_POST['search-rules-acp-categorias-advanced'])) !== false)) {
						$liberarPUSH = true;
				}
			} elseif (isset($_POST['searchSIDThreads']) && $_POST['searchSIDThreads'] == trim($sid)) {
				$liberarPUSH = true;			
			} else {
				$msg = explode(" ", $msg)[0];
				if (strpos("(", $msg) >= 0) {
					$msg = explode("(", $msg)[0];
					$msg = explode("-", $msg)[0] . "-" . explode("-", $msg)[1] . "-" . explode("-", $msg)[2];
				}
				$allACPMsg[] = $msg;
			}
			if ($liberarPUSH) {
...
if (isset($_POST['pesquisarCategoria']) || isset($_POST['search-rules-acp-categorias-advanced'])) {
	asort($allMsgs);
} else {
	...
	<div class="step-content">
		<div class="panel-body">
			...
			<div class="panel panel-default" style="margin-top:20px">
				<?php if (isset($_POST['pesquisarCategoria']) || isset($_POST['search-rules-acp-categorias-advanced']) || isset($_POST['searchSIDThreads'])) { ?>
					<hr>
						<div class="infoblock">
							<div class="alert alert-info clearfix" role="alert">
								<div class="pull-left">
									<p><?=gettext("Threat state change")?></p>
									<hr>
									<p><?=gettext("This page makes it possible to change the status of the threats listed depending on your need, remembering that care is needed on this page, as it is directly dealing with the 'threats' that Active protection uses to maintain security on the network.")?></p>
									<dl class="dl-horizontal responsive">
										<dt><?=gettext('Sobre os estados')?></dt><dd></dd>				
										<dt><i class="fa fa-check"></i></dt><dd><?=gettext("Pass - This option causes this 'threat' to be ignored, giving you access to the internal network");?></dd>
										<dt><i class="fa fa-info"></i></dt><dd><?=gettext("Alert - This option alerts about the access attempt, preventing access, but returning that access was denied to the threat");?></dd>
										<dt><i class="fa fa fa-times"></i></dt><dd><?=gettext("Drop - This option completely blocks the threat from accessing the internal network");?></dd>
										<dt><i class="fa fa-cog"></i></dt><dd><?=gettext("Save - If the threat has suffered a state change, this is the option that will save the new state and apply it to the Active protection system");?></dd>
										<dt><i class="fa fa-cog"></i></dt><dd><?=gettext("Restore Threat - If the threat has undergone any state change, this option will restore the threat to the original state before the change, it is worth noting that after saving, the new saved state will be the reset state if the threat undergo a new change");?></dd>
									</dl>
									<p><?=gettext("Note: The search filter is only valid for the threats listed within the table, to search for other unlisted threats, return to the main menu by clicking the red X in the upper right corner")?></p>
								</div>
							</div>
						</div>
					<hr>
					<div class="panel panel-default" style="margin-top:20px">
						...
						<?php if ((isset($_POST['search-rules-acp-categorias-advanced']) && is_numeric($_POST['search-rules-acp-categorias-advanced'])) || (isset($_POST['searchSIDThreads']) && is_numeric($_POST['searchSIDThreads']))): ?>
								<?php if ((isset($_POST['searchSIDThreads']) && is_numeric($_POST['searchSIDThreads']))): ?>
									<h2 style="margin: 0px; font-size: 18px;margin-top:12px;"><?=gettext("Rules")?> Active Protection - SID: <?=$_POST['searchSIDThreads']?></h2>
								<?php else: ?>
									<h2 style="margin: 0px; font-size: 18px;margin-top:12px;"><?=gettext("Threats")?> Active Protection - SID: <?=$_POST['search-rules-acp-categorias-advanced']?></h2>
								<?php endif; ?>
							<?php else: ?>
								<h2 style="margin: 0px; font-size: 18px;margin-top:12px;"><?=gettext("Threats")?> Active Protection - <?=$_POST['pesquisarCategoria'] . $_POST['search-rules-acp-categorias-advanced']?></h2>
							<?php endif; ?>
							...
							<form action="./services_acp_ameacas.php" method="POST" style="margin: 0px; padding: 0px; border: 0px solid transparent; width:5%;">
								...
								<table class="table table-bordered" id="table-access">
									<thead>
										<tr>
											...
											<th style="vertical-align: inherit !important; background: #177bb4 !important; color: white !important;"><i class="fa fa-navicon"></i> <?=gettext("Websites/Applications/Services")?> - <?=gettext("Threats")?>: <p id="contadorRules" style="display: contents;"><?=$contadorFiltrado?></p></th>
											<th style="vertical-align: inherit !important; background: #177bb4 !important; color: white !important;"><?=gettext("Actions")?><br>
										</tr>
									</thead>
									...
									foreach ($allMsgs as $array_key => $array_value) {
										...
										echo "<button style='margin-right: 5px;margin-top: 5px;' onclick='resetTarget(" . "\"". 'linhaStatus_' . $array_key . "\"" . ", " . "\"". $allStatus[$array_key] . "_" . $array_key . "\"" . ")' id='btn_reset_" . $array_key . "' class='btn btn-secondary btn-secondary-change-state no-confirm btn-disabled' disabled><i class='fa fa-cog'></i> " . gettext('Reset threats') . "</button>";
										echo "</th></tr>";
									}
									?>
									</tbody>
						...
						<div class="infoblock">
							<div class="alert alert-info clearfix" role="alert">
								<div class="pull-left">
									<p><?=gettext("Threat research")?></p>
									<hr>
									<ul>
										<li><?=gettext("Simple method - Performs only the threat search by a certain category")?></li>
										<li><?=gettext("Advanced method - Performs a search for all threats that match the description you are looking for")?></li>
									</ul>
									<p><?=gettext("Note: The search field changes according to the search, the simple mode only returns categories with compatible search, when advanced mode is activated, the search will return all threats compatible with the typed one, remembering that in advanced mode it is required at least 5 input characters to filter the threats more precisely")?></p>
								</div>
							</div>
						</div>
						<hr>
						<div class="panel-body" style="display: inline-flex;border:0px solid transparent; width: 100%;">
							<h2 style="margin: 0px; font-size: 24px;margin-top:12px;" id="desscriptionFilter"><?=gettext("Threats")?> Active Protection - <?=gettext("Simple")?></h2>
							<div id="simpleFindThreats" style="margin: 0px; padding: 0px; border: 0px solid transparent; width: 45%;margin-left: auto; display: flex;">
							...
							<form action="./services_acp_ameacas.php" method="POST" id="advancedFindThreats" style="margin: 0px; padding: 0px; border: 0px solid transparent; width: 45%;margin-left: auto; display: none;">
						...
						<div class="panel-body" style="text-align: center;" id="painelFilterCategory">
							...
							<form action="./services_acp_ameacas.php" method="POST" style="border:0px solid transparent !important;">
	...
	<div class="modal-dialog  modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" style="font-size:16px;" onclick="showNoneModalPass()"><?=gettext("Return the Threats")?></button>
				<button type="button" class="btn btn-primary" style="font-size:16px;" onclick="applicateInInterfacesACP()"><?=gettext("Apply Threats")?></button>
			</div>
		</div>
	</div>
</div>
...
function searchButton() {
	...
	if (val.length == 0) {
		...
		if (displayInFlex > 0) {
			document.getElementById("resultadoSimpleFilter").textContent = "<?=gettext("Results found")?>";
		} else {
			document.getElementById("resultadoSimpleFilter").textContent = "<?=gettext("No results found")?>";
		}
	}
}
...
function searchDataRules(msgCustom = "") {
	...
	if (msgCustom.length > 0) {
		...
		$('#modal_ativa_regras .txt_modal_ativa_regras').text("<?=gettext("Search Threats")?>");
	}
...
function alterarEstado(valor) {
	if ((estadoASerAlterado == valor.split("_")[1]) || (estadoASerAlterado == "")) {
		...
		$('#modal_ativa_regras .txt_modal_ativa_regras').text("<?=gettext("To change other threats, it is necessary to apply the changes already made to the threats (Click to go to the threats):")?>");
...
function searchAdvanceValue() {
	...
	document.getElementById("search-rules-acp-categorias-simple").value="";
	document.getElementById("simpleFindThreats").style.display = "none";
	document.getElementById("advancedFindThreats").style.display = "flex";
	document.getElementById("desscriptionFilter").textContent = "<?=gettext("Threats")?> Active Protection - <?=gettext("Advanced")?>";
}
...
function closeAdvanceSearch() {
	...
	document.getElementById("search-rules-acp-categorias-advanced").value="";
	document.getElementById("simpleFindThreats").style.display = "flex";
	document.getElementById("advancedFindThreats").style.display = "none";
	document.getElementById("desscriptionFilter").textContent = "<?=gettext("Threats")?> Active Protection - <?=gettext("Simple")?>";
}
...
function saveTarget(pegarValor) {
	...
	$('#modal_ativa_regras #loader_modal_ativa_regras').attr('style', 'width:200px;height:200px');
	$('#modal_ativa_regras #loader_modal_ativa_regras').attr('src', '../images/spinner.gif');
	$('#modal_ativa_regras .txt_modal_ativa_regras').text("<?=gettext("Updating Active Protection threats")?>");
	$('#modal_ativa_regras .txt_modal_ativa_real_rule').text("");
	$('#modal_ativa_regras').modal('show');
	...
	setTimeout(() => {
		...
		$.ajax({
			data: {'save_ameacas_ext': document.getElementById(pegarValor).value},
			method: "POST",

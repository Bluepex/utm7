	...
	protected function _getInput()
	{
		...
		if (!empty($this->_description) || is_numeric($this->_description)) {
			if (!empty($pkg_intl_translate)) {
				$description = htmlspecialchars(dgettext($pkg_intl_translate, $this->_description)) .'</label>';
			} else {
				$description = htmlspecialchars(gettext($this->_description)) .'</label>';
			}

			return '<label class="chkboxlbl">'. $input .' '. $description .'</label>';
		}

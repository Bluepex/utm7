...
class Form_Group extends Form_Element
{
	...
	public function __construct($title, $hidden = false)
	{
		$this->_title = $title;

		if($hidden === true){
			$this->_attributes['class']['group_hidden'] = true;
		}
	}
	...
	public function setHelp()
	{
		...
		if (($arg0_len > 0) && ($arg0_len < 4096)) {
			if (!empty($pkg_intl_translate)) {
				$args[0] = dgettext($pkg_intl_translate, $args[0]);
			} else {
	...
	protected function _getHelp()
	{
		...
		$group->addClass('col-sm-'. Form::MAX_INPUT_WIDTH);
...
EOT;
	...
	{
		global $config, $user_settings, $pkg_intl_translate;
		...
		$label->addClass('col-sm-'.Form::LABEL_WIDTH, 'control-label', 'blue-title');

...
class Form_Section extends Form_Element
{
	...
	public function __construct($title, $id = "", $collapsible = 0, $hidden = false )
	{
		if (!empty($id)) {
			$this->_attributes['id'] = $id;
		}
		if($hidden === true){
			$this->_attributes['class']['section_hidden'] = true;
		}
	...
	public function addInput(Form_Input $input, $hidden = false )
	{
		$group = new Form_Group($input->getTitle(), $hidden);
		$group->add($input);
	...
	{
		global $pkg_intl_translate;

		$element = parent::__toString();

		if (!empty(trim($this->_title)) || is_numeric($this->_title))
			if (!empty($pkg_intl_translate)) {
				$title = htmlspecialchars(dgettext($pkg_intl_translate, $this->_title));
			} else {
				$title = htmlspecialchars(gettext($this->_title.$pkg_intl_translate));
			}
		else
			$title = '';
		...
		if (intval($this->_collapsible) & COLLAPSIBLE) {
			$hdricon = '<span class="widget-heading-icon">' .
				...
				'</a>' .
			'</span>';
			$bodyclass = '<div id="' . $id . '_panel-body" class="mt-4 collapse ';
			$divBody = '</div>';
...
EOT;
	...
	<div class="row mb-4 mt-2">
		<div class="title-description pb-5">
			<h5 class="text-color-blue">{$title}{$hdricon}</h5>
		</div>
		{$bodyclass}
			{$body}
		{$divBody}
...
EOT2;
	...
	<div class="row mb-4 mt-2">
		<div class="col-12 col-md-12 col-xl-12">
			{$element}
				<div class="title-description pb-5">
					<h2 class="text-color-blue"><a name="{$id}">{$title}</a>{$hdricon}</h2>
				</div>
				{$bodyclass}
					{$body}
				</div>

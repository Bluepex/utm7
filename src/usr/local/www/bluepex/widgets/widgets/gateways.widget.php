...
if (!function_exists('compose_table_body_contents')) {
	function compose_table_body_contents($widgetkey) {
		...
		foreach ($a_gateways as $gname => $gateway) {
			...
			$gw_displayed = true;
			$rtnstr .= "<tr style='cursor:pointer;' onclick=\"window.location.href='link_monitor.php?link=" . htmlspecialchars($gateway['name']) . "'\">\n";
			$rtnstr .= "<td title='{$title}' style='vertical-align:middle;'><i class='fa {$icon}'></i></td>\n";
			$rtnstr .= "<td title='{$gtitle}' style='vertical-align:middle;'>\n";
			...
			if ($gateways_status[$gname]) {
				$title = "Ótimo";
				$icon = "great-icon.png";
				$packets_loss = htmlspecialchars($gateways_status[$gname]['loss']);
				$latency = htmlspecialchars(number_format((float)rtrim($gateways_status[$gname]['delay'], "ms"), 1));
				if ($packets_loss > 10 || $latency > 420) {
					$icon = "very-bad-icon.png";
					$title = "Muito Ruim";
				} else if (($packets_loss > 1 && $packets_loss <= 10) || ($latency > 180 && $latency <= 420)) {
					$icon = "bad-icon.png";
					$title = "Ruim";
				} else if (($packets_loss > 0 && $packets_loss <= 1) || ($latency > 60 && $latency <= 180)) {
					$icon = "regular-icon.png";
					$title = "Regular";
				} else if (($packets_loss == 0) && ($latency > 20 && $latency <= 60)) {
					$icon = "nice-icon.png";
					$title = "Bom";
				}
				$online = "<a href='#' title='{$title}'><img src='./images/{$icon}' height='25'></a>";
			} else {
				$icon = "very-bad-icon.png";
				$title = "Muito Ruim";
				$online = "<a href='#' title='{$title}'><img src='./images/{$icon}' height='25'></a>";
			...
			$rtnstr .= '<td style="vertical-align:middle;">' . $online . "</td>\n";
			$rtnstr .= "</tr>\n";
		}

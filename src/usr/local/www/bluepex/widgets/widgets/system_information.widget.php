...
$hide_items = array("bios", "platform", "cpu_type", "hwcrypto");
...
$skipsysinfoitems = array_unique(array_merge($skipsysinfoitems, $hide_items));
...
?>
		...
		<?php if(file_exists('/etc/serial')) : ?>
		<tr>
			<th><?=gettext("Serial")?></td>
			<td><?php readfile("/etc/serial"); ?></td>
		</tr>
		<?php endif; ?>
<?php
	endif;
...
<?php
				foreach ($sysinfo_items as $sysinfo_item_key => $sysinfo_item_name):
					if (in_array($sysinfo_item_key, $hide_items)) {
						continue;
					}
?>
						<tr>

			...
			foreach ($results as $result) {
				if ($counter == 1) {
					print ("<li>{$result}&emsp;<i class=\"fa fa-trash icon-pointer\" id=\"pfblockerngackicon\"
						title=\"" . dgettext("pfBlockerNG", "Clear Failed Downloads") . "\" ></i></li>");
				} else {
					print ("<li>{$result}</li>");
				}
				$counter++;
...
?>
		...
		<table class="table table-condensed">
			...
			<tbody>
				<tr>
					<td>
						<i class="<?=$pfb_status?>" title="<?=dgettext("pfBlockerNG", $pfb_msg)?>"></i>
					</td>
					<td>
						<?php if ($dcount != 0): ?>
					...
					<td>
						<a target="_blank" href="pfblockerng/pfblockerng_log.php" title="<?=dgettext("pfBlockerNG", "Click to open Logs tab")?>">
				...
				<tr>
					<td>
						<i class="<?=$dnsbl_status?>" title="<?=dgettext("pfBlockerNG", $dnsbl_msg)?>"></i>
					</td>
					<td>
					...
					<td>
						<i class="fa fa-trash icon-pointer" id="pfblockerngdnsblclearicon" title="<?=dgettext("pfBlockerNG", "Clear DNSBL Packets")?>"></i>
					</td>
				</tr>
				<?php endif; ?>
			</tbody>
	...
	<div class="table-responsive">
		<table id="pfb-tbl" class="table table-striped table-hover table-condensed sortable-theme-bootstrap" data-sortable>
			<thead>
				<tr>
					<th><?=dgettext("pfBlockerNG", "Alias");?></th>
					<th title="The count can be a mixture of Single IPs or CIDR values"><?=dgettext("pfBlockerNG", "Count");?></th>
					<th title="Packet Counts can be cleared by the pfSense filter_configure() function.
						Make sure Rule Descriptions start with 'pfB_'"><?=dgettext("pfBlockerNG", "Packets");?></th>
					<th title="Last Update (Date/Time) of the Alias"><?=dgettext("pfBlockerNG", "Updated");?></th>
...
<form action="/widgets/widgets/pfblockerng.widget.php" method="post" class="form-horizontal">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			...
			<?php foreach (array('none' => dgettext("pfBlockerNG", 'None'), 'alias' => dgettext("pfBlockerNG", 'Alias'), 'count' => dgettext("pfBlockerNG", 'Count'),
			    'packets' => dgettext("pfBlockerNG", 'Packets'), 'updated' => dgettext("pfBlockerNG", 'Updated'))
				as $sort => $sorttype):?>
	...
	<div class="form-group">
		<label for="pfb_sortdir" class="col-sm-8 control-label"><?=dgettext("pfBlockerNG", 'Select sort direction')?></label>
		<div class="col-sm-4">
			<label><input type="radio" name="pfb_sortdir" id="pfb_sortdir_asc" value="asc"
				<?=($pfb['sortdir'] == "asc" ? 'checked' : '')?> /> <?=dgettext("pfBlockerNG", 'Ascending')?></label>
			<label><input type="radio" name="pfb_sortdir" id="pfb_sortdir_des" value="des"
				<?=($pfb['sortdir'] == "des" ? 'checked' : '')?> /> <?=dgettext("pfBlockerNG", 'Descending')?></label>
		</div>
	</div>
	<div class="form-group">
		<div class="col-sm-offset-4 col-sm-8">
			<button type="submit" name="pfb_submit" id="pfb_submit" class="btn btn-primary">
				<i class="fa fa-save icon-embed-btn"></i><?=dgettext("pfBlockerNG", 'Save Settings')?>
			</button>
		</div>
	</div>
</form>
